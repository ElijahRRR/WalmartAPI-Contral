...
- **首跑 dry-run 必须人眼核对两处**(报告打了标记,但没人看 = 白打):
  ①「将改名」清单 —— 对上但拼写不同的存量行(含 Drugs & Paraphernalia 一族缩写名)
  会被改为官方拼写,逐条确认确实是同一政策类别;
  ②「未对上」清单 —— 真正对不上的官方类别(如武器族)会新增行,表有官方无的行只报告不删,
  逐条裁决新增/忽略;
- **`AUDIT_RULES_VERSION` 已随改名批递增至 `c.2026-09-02.1`,首跑无需再手动递增**
  (政策表内容变化 = L3 判定输入变化;若合并那批之后**又改了判据**,那次要自己再提一版);
  ⚠ **成本**:政策表一改,L3 的 system prompt 就逐字节变化 ⇒ `catalog.llm_cache` 里
  L3 那一批**全量未命中**,与随之而来的全量重审叠加 = 那批产品**全额重付**;
  大批重审排北京时间 18:00–次日 08:00 的谷时段跑,单价减半(摘要会点名);
- 真跑摘要还会点名另一条连带后果:新增行人工中文列全 NULL(S4 现渲染为空壳标题,
  待运营补中文)。原先那条「`audit_l3.py` S1/S3 硬写 37 条」已随改名批**动态化** ——
  提示词按实时政策条数渲染,不再有对不上的字面量;
- 喂 LLM 的"机器喂入版"不落库,由渲染层从 full_policy 派生(剥 URL/导览/免责声明/头注/chrome,
  详见 `docs/policy_sync.md` §十)。

## 第 6 步 · 飞书投影 + 归类对照复跑

- 飞书单向投影(PG → 飞书 A-E)见 `docs/policy_sync.md` §八.2;**回同步通道已取消**(2026-09-02),人工区只在飞书;
