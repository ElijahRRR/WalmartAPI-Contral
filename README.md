...
- **83 条工作流**,覆盖订单、产品数据、审核、上架、维护清理、风控黑名单、
  类目映射、店铺分配、KPI 日报八个业务域;
- **13 条自动任务**在生产运行(电脑 launchd 4 条高频 + 智能体定时任务 9 条每日/每周);
- **PostgreSQL 17** 单库五 schema(56 表 / 14 视图)为唯一权威状态;
- **3081 个单元测试**(3033 跑 + 48 跳过;`python -m pytest -q` 的实数,**随批次手工同步、没有守门测试** —— 漂了不会红,改动后请重跑一遍再改这个数)。
...
| `item_id_sync` | 调 | 用沃尔玛 On-request **ITEM 报表**补 `walmart_items.item_id`(数字 itemId 只有报表批量给)。每天只为有缺口的店各拿一份报表(创建每店每小时一次、生成 15–45 分钟),先落台账 `ops.report_requests` 再调接口,超时/限流下轮接着;写库**报表为准**;首轮 `-p all=1`,探针 `-p store=X -p probe=1`。飞书 itemId 列仍由 catalog_sync 投影 |
...
| 工作流 | | 做什么 |
|---|---|---|
| `list_new` | 危 调 | 上架主链。七道闸门(非 ACTIVE 店 / 配额 / 风控 / 黑名单 / **本店去重**(2026-08-28 取消全局去重:同店重复才拦,跨店分布归分配链+占用闸)/ 防呆 / 数据过滤,含**店铺渠道闸**:限额表「配送限制」标了 fba/fbm 就只上该渠道的货,**没标不限制**;另有**定制品闸**:产品数据标为定制的不上架(2026-08-28 定稿);**库存门槛与最大库存**:门槛决定上不上,亚马逊库存 <5 不上架(理由「亚马逊库存不足5」);限额表「最大库存」N 决定上多少,上架数量 = min(亚马逊库存, N)(2026-09-25 定稿,换算与维护同一个函数 `store_limits.stock_for`);缺数据同轮闭环:推采集→等窗口→就地摄取→本轮续走)→ 预备期(LLM 属性映射 + spec 一致化,128 并发占位号,缺必填本地拦)→ **通过的行才领 UPC** → 按店打包提交 MP_ITEM feed(三条防线:起跑抖动 0~800ms 去同步 / 遇 5xx 并发按 24→16→12→8→4 降档、只降不升 / 不确定的片子**整轮跑完再结算**,退避走官方阶梯 2-4-8-16-32 + 抖动)→ 回写上架表。**首尾各同步一次 UPC 池**,等于顺带跑了 `upc_sync`。变体自动成组(单维/多维)、组内标题差异化 摘要末尾报本轮 **LLM 用量与花费**(按用途分行,含每千条单价;dry-run 的 `check_spec` 预检同样报——那一步是真调 LLM 的)。|
...
| 工作流 | | 做什么 |
|---|---|---|
| `maintenance_scan` | 调 | 扫描定性,产建议行(改价 / 改库存 / 改标题 / 删除)。判据全在 `services.maintenance_intents.classify()` —— 全项目唯一一处决定"这个在线商品该拿它怎么办"。改库存跟随亚马逊时:<5 写 0(所有店),过了门槛写 min(亚马逊库存, 限额表「最大库存」N)(2026-09-25;首行报「封顶 N」) |
...
| `node_clear` | 危 调 | 把**受管仓以外**的节点库存清零(搬仓收尾,一次性,不进调度):切到受管仓后旧节点的存量货自动链一律不碰,等受管仓充起来再用它清空旧仓。**旧仓编号自动发现**(`GET /v3/inventories` 的 shipNode,2026-09-25),不带 `-p store` = 所有填了「维护仓库」且校验通过的店;`-p node=` 只清某一个旧仓。只清受管仓已接管的 SKU(`-p include_untaken=1` 才全清)、**拒绝清受管仓**、没给 shipNode 的数量不碰;写 0 幂等,失败重跑即补 |
...
| `item_id_sync` | 每天 05:00 | `item_id_sync`(有缺口的店各拿一份 ITEM 报表补 item_id;首轮手动 `-p all=1`) |
