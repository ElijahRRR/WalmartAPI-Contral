...
| 工作流 | | 做什么 |
|---|---|---|
| `maintenance_scan` | 调 | 扫描定性,产建议行(改价 / 改库存 / 改标题 / 删除)。判据全在 `services.maintenance_intents.classify()` —— 全项目唯一一处决定"这个在线商品该拿它怎么办"。改库存跟随亚马逊时:<5 写 0(所有店),限额表「最大库存」N 的店 ≥N 写 N、<N 写 0(2026-09-25;首行报「封顶 N」) |
