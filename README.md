...
| 工作流 | | 做什么 |
|---|---|---|
| `list_new` | 危 调 | 上架主链。七道闸门(非 ACTIVE 店 / 配额 / 风控 / 黑名单 / 全局去重 / 防呆 / 数据过滤)→ 领 UPC → LLM 属性映射 → spec 一致化 → 提交 MP_ITEM feed → 回写上架表。**首尾各同步一次 UPC 池**,等于顺带跑了 `upc_sync`。变体自动成组(单维/多维)、组内标题差异化 |
