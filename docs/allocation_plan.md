...
1. 候选店 = 过全部硬闸的 ACTIVE 店。类目闸:组主大类 ∈ 店已有类目;店类目<2
   ...
   一批内一家店最多开 1 个新大类(→ Q5)。**渠道闸**(§十二.12②):产品渠道
   = 店已锁渠道;店未锁渠道时"锁定渠道"同开新类目一样走方案表高亮提议;
...
- 产品侧渠道来源:`latest_snapshot.raw->>'is_fba'`(list_new ⑥c 既有口径,
  ...
  "渠道未知不定价"一脉);
- 店铺侧登记:A1 新增 `catalog.store_channel`(store PK / channel FBA|FBM /
  claimed_at)——与 store_categories 同一套纪律:**分配 execute 时锁定**
  (店首次接货即锁,方案表高亮「将锁定店 X 渠道=FBA」),此后只有
  `store_release --store X` 整店释放时清空,无任何自动改写;
...
- **A0.5 存量冲突审计 + 过渡方案**(只读报告,不写 claims;A0.1 后即可跑):
  ...
  §十二.11)⑥ 每店在线商品渠道分布(FBA/FBM 混杂店进过渡清单,§十二.12②)。
...
- **A1 占用台账地基**(§八 A1 + 两处追加:claims + store_categories +
  **store_channel(§十二.12②)** + services/claims + list_new 占用闸 +
  store_release(整店释放同步清 store_channel + 标 walmart_items
  missing_since,§十二.11)+ 审计对拍)。
