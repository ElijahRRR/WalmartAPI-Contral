...
**缺数据 / 数据过旧同轮补采**(判据 2026-09-10 所有者定稿):领到待审 ASIN 后先看
重审开关 —— 重审关(缺省)推「库里没有结论(未审/pending)且 12 小时内没有快照」的,
重审开(`-p force=1`)推「12 小时内没有快照」的全部;不在库 / 有行无标题的行没结论,
自然在第一档;12 小时内采过的(不分结局)一律不重推,这同时是失败重推的冷却期。
批次名 `audit_gap_<北京时间戳>`(按时间不按日界:日界名会让同日再跑时新加的品撞名
409 推不出去)→ 插队 → 轮询等采完(缺省 60 分钟)→ 就地摄取 → **采回来的这一轮就
判掉**;没等到的在库用现值审,审不了的把真实原因(推送失败 / not_found 等结局 /
采集侧 error_type / 没等到)写进表格「具体内容」列,「审核结果」留空 ⇒ 下轮重领。
...
> ⚠ **小批量跑出来的命中率低,是算术不是缺陷**(2026-09-03 所有者问到,记在这里
> 免得下次又当成 bug 查)。冷启动次数 `k` 是个小常数(串行预热那一条,加上并发
> 请求落到 DeepSeek 另一个节点时那一两条 —— 前缀缓存按节点存),于是
> **命中率 ≈ (N − k) / N**,N 是本轮的 L3 调用数。8 个产品(7 次调用)那一轮
> 实测 73%,反算 miss 折合 1.9 次整调用 ⇒ k ≈ 2,与公式吻合;同一批 prompt 换到
> N=1000 就是 99.8%。**改提示词结构提不了这个数** —— 固定内容已经 100% 在 system
> 段、全部排在可变内容之前,可变的 user 段只占每次约 300–700 token(<2%)。
>
> 但这个数**很值钱**:`deepseek-v4-flash` 的 miss 单价是 hit 的 **50 倍**
> (高峰 2 元 vs 0.04 元 / 1M token,见 `registry.LLM_PRICING`)。
> **所以要省钱就攒批 + 排谷时段(含周末全天),别动提示词。**
> ⚠ 下面这组绝对值是 **2026-09-10 调价前**按美元表推的,只看趋势别当报价:
> 峰值 N=7 ≈ $6.2/千条 → N=50 ≈ $1.4 → N=200 ≈ $0.84 → N=1000 ≈ $0.68。
> 新价(元)下 hit 降 60%、miss 降 33%、**出只降 11%**,而 L3 这种"大前缀命中 +
> 千级输出"的形态里输出占大头 ⇒ 总额降约 25–30%,远小于命中价那 60%。
> ⚠ `audit_replay --dry-run` 的预估按 **k=1** 算(首条未命中、其余全中),
> 实测 k≈2,所以它偏乐观一点点;量级是对的。
