...
L3 判 7 条(`B0015XDK30` 在 **L0 商标符号硬拒**就停了,没进 L3)。**8 条对上 6 条**:
...
| `B0C1V8SVHZ` | reject(后台要儿童证书) | **pass** | 印有西部牛仔图案的装饰门垫(家居用品) | ❌ 未达 |
...
- **`B0GYNRCZ9F` 是 `.2` 自己弄出来的回归** —— 所有者要的是「假品牌无需
  **输出**」,`.2` 顺手写成了「无需逐个**判**」;判定动作一松模型整段跳过,
  ...
  这一维修掉(见版本日志),**不是给 `abba` 开特例**;
- **`B0C1V8SVHZ` 不是回归,是本来就没接住的一条**。所有者的依据是"沃尔玛
  **后台**显示要儿童证书",而新链判的是本体:一张西部牛仔图案的装饰门垫,
  按政策原文确实不是儿童产品。这里要分清两件事 ——「这个**产品**要不要
  CPC」(政策问题,现在判的是它)与「沃尔玛对这个 **PT** 的准入闸门要不要
  CPC」(运营事实,`walmart_pt_meta.requirements` 那一列)。后者若确实卡着
  这条 listing,那是 `类目准入` 的活;**但它与 2026-08-20 删 R3 时定下的
  「不按类目名连坐整类」直接冲突**,该怎么判是所有者的裁决,不是提示词能
  自己决定的 —— 裁决前一律按现状(拿不准不拒)。
  待办:`python cli.py audit_why -p asins=B0C1V8SVHZ` 看它的 PT 那行准入要求
  到底写了什么,再拿事实去请裁决。
