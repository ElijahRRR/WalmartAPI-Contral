...
| `audit.walmart_category_map` | 已有映射(高置信行) | 13,233 node |

三方 JOIN 命中率实测(`taxonomy_import` 预览**强制**先给这个数,不看命中率
就导入=重蹈 Best Sellers 那次覆辙):产品侧 82.2%,映射表侧 99.9%。

**类目树目前只有叶子层**:对账版 JSON 的 `meta.reconciled_tree_rows` 是 32,147,
实发 28,495(leaves 26,956 + unverified_new_nodes 1,539,verified_added_paths 0 条),
差的约 5,191 是中间层非叶节点,文件里没有。当前用法只按叶子 node 查,不影响
②a 直查;但**父链兜底做不了**(想做"叶子无映射就退一级找父类目"要先补中间层)。
...
映射覆盖率 61.5% → 62.9% → **63.7%**(产品侧 15,538 node 已映射 9,895)。
冲突行由 `catmap_fix` 定点修正:旧行降级为'低'并留痕(**不删**,旧映射是历史证据),
新行带 node_id 以 `mined_conflict_fix` 插入。

**这一段是常态化重跑,不是一次性**:A 桶还剩 1,856 node / 17.1 万件"有实证 PT
