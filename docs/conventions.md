...
- **真兜底**(外部 API 自身缺口,如 ITEM 报表兜底在线品、飞书挂走快照)→ 三要件:
...
**通知文本也有上限**(2026-09-10 补):飞书**文本消息**官方上限 150 KB(卡片/富文本
30 KB),超了**整条被拒**(`code=230025`),不是截断发出。本仓取 ×95% =
145,920 **字节**(`_MESSAGE_TEXT_MAX_BYTES`,单位是字节不是字符 —— 一个汉字 UTF-8
占 3 字节)。`api/feishu.notify` 在**应用直发与 webhook 分叉之前**过这道闸:超长
**保首行 + 尾部截断说明**(首行是链通知的唯一内容,cli 只取它),截断记
`logger.warning` 不静默。实证:A131吕灿荣 `sku_migrate -p settle_only=1` 一轮几百条
明细逐条各占一行,那轮摘要一个字都没推出去(见 docs/sku_plan.md §9.15 八)。
⚠ 闸是保险丝不是主防线:摘要里"不用人动手"的信息该**归并成一行**,别逐条报。
...
| 230025 | **发消息**超:消息体长度超限(文本 150 KB / 卡片富文本 30 KB) | 整条被拒,不是截断发出 ⇒ `notify` 发之前按 `_MESSAGE_TEXT_MAX_BYTES` 保首行截尾 + 记 warning;真正的修法是把摘要写短(信息类归并成一行) |
