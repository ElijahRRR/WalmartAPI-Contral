...
### 换肤返工(同日):token 硬映射 → 组件级重绘

第一版换肤把 accent 变量直接映成 zinc-900 黑 —— 但原型把一个蓝 accent 用在
72 处、承担七种角色(链接/导航高亮/区块标/强调数字/中间态签/选中态/主按钮),
一刀切等于把七种角色全砸成黑灰,层级与对比全毁(所有者验收打回)。重做:

- **按角色拆色**:信息/链接/强调 → sky(#0369A1 系);主动作/激活导航 →
  zinc-900 黑(独立组件规则,不走 accent);危险 → red-600 实心(替换原型的
  斜纹描边语言);语义三组 red/amber/emerald(50 底/700 字/200 线);中间态
  sky 虚线(与「忙」的灰虚线必须分得开)。
- **组件皮肤层**(级联末位覆盖):白卡圆角 8 / 按钮输入 6 / 签 4,黑主按钮
  (hover zinc-800)、黑选中分段、erp 表格解剖(表头 #FAFAFA + #71717A 弱灰
  小写大写字距)、激活导航黑底白字(深色反白,navGroups 按主题出色)。
- **深浅两套**全部显式定义;蓝图四角标记隐藏。
- **顺手修了原型三处数据错**:订单表审核列取 r[11] 越界(值在 r[10],整列
  兜底成「建议拒绝」)、危险计数 21→20、「其余 55 条手动」混淆任务与工作流
  (11 条任务共跑 21 条工作流,手动 45 条)。
- **发布前 Playwright 实渲染截图自验**:总览/发令台/订单/审核/类目映射/
  深色/危险抽屉七张过目后才发布 —— 此前两版盲发的教训。
