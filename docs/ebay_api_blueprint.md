...
## 1. 端点清单(44 组 / 62 个端点,只列本项目工作流会用到的)
...
| 2 ebay_taxonomy_sync | False | 9,(10),(11),12 | 每日只调 9 比版本,`categoryTreeVersion` 变了才拉 10/11。全量落库,上架时走库。⚠ **本链只同步 eBay 侧类目树,不产出「亚马逊 node → eBay categoryId」的映射** —— 那条映射链(沃尔玛侧对应 `catmap_*` 九条工作流 + `docs/category_mapping.md`)**一期不链化**:`docs/ebay_plan.md` 判据 ⑨ 与 §七 #11 已定「**一期由所有者手填 `audit.ebay_category_map`,`import/suggest/promote` 三条链列二期**」,填表规范与 fail-closed 写在该文批次 2b;api 面已就绪(#12/#13),二期立链时直接用 |
...
| 13 ebay_ship_confirm | True | 31 | **标发货回传运单**:一包裹一次 `createShippingFulfillment`,提交前先 `GET` 回读防重复标发。⚠ **本行是校验轮补上的**:原矩阵里 #31 只出现在只读的 4 `ebay_submit_poll`,等于台账里的 `fulfillmentId` **永远不会被写入**,而 submit_poll 会去反查一个从未提交过的东西。运单回传是 late shipment / INR 绩效的命门,**不能没有生产者**。⚠ **批次归属与工时归 `docs/ebay_plan.md`,已立于该文批次 6** |
| **14 ebay_authorize** | True | 1,2 | **一次性人工触发的授权兑换链,不进 `schedule.JOBS`**(与 1 `ebay_bootstrap_account` 同类):拼 consent URL(#2,`redirect_uri` 填 **RuName**)→ 收人工粘回的 code → 调 `exchange_code`(#1 的 `authorization_code` grant,**全仓唯一一处**,§4.2 #7)→ 落 `ops.ebay_tokens` → 打印 `refresh_expires_at`。⚠ **本行是第二轮校验补上的**:#1 的 `authorization_code` grant 与 #2 的 consent 页在原矩阵里**一条工作流都没认领**,而 §4.2 #7 与 §7 都已定稿要有 `exchange_code` ⇒ 正撞本文自己给 #31 用过的那条判据「**端点不能没有生产者**」。⚠ **它不计进上面那 13 条业务链**(那 13 条是进调度或按业务触发的);批次归属与浏览器 runbook 归 `docs/ebay_plan.md`,已立于该文批次 3(含 `docs/ebay_runbook.md`) |

**对给定清单的五处增删,逐条给理由**:**拆出 9 `ebay_problem_cleanup`** —— withdraw/delete 是破坏动作,CLAUDE.md 08-24 定稿「破坏动作只有一个出口」「破坏组存在即压制同 SKU 的维护组」,塞进 `ebay_maintenance` 等于把 08-19 那条"说不清是哪条链干的"事故重演一遍。**拆出 10 `ebay_returns_action`** —— `ebay_returns_sync` 是只读同步而 decide/refund 是不可逆写。⚠ **拆分理由已按仓库实际改正(2026-08-25)**:`cli.py:297-305` 恒同时注入 `execute` 与 `dry_run` 两个键(`params["execute"] = (not dry_run) if dangerous else True`;`params.setdefault("dry_run", bool(dry_run))`),`DANGEROUS=True` 时二者等价 ⇒ **原稿写的"两套判据不能共存于一个 `run()`"是错的**。正确理由是:**`DANGEROUS` 是模块级开关**,混在一条链里会把只读同步的那半边一起标成危险链(整链进 `--dry-run` 纪律与破坏动作审计口径),且**破坏动作必须只有一个出口**。**记下错理由本身是为了防止下次有人问"能不能合并"时拿它给出错误答案。** **新增 13 `ebay_ship_confirm`** —— 见上表,#31 原先没有生产者。**新增 12 `ebay_account_health`** —— 三件事合并成一条,理由是它们**同为每日一次、只读、告警型**;其中 refresh token 巡检是 **eBay 独有的必需品**:OAuth **没有** `HardExpirationWarning`(那是 Auth'n'Auth 旧令牌才有的 7 天预警),eBay 不会提醒你 refresh token 快过期。⚠ **refresh token 失效必须是显式停链告警状态,绝不能让 workflow 静默跳过该账号** —— 否则重演 CLAUDE.md 里"每天空转而且报成功"那类事故。**新增 14 `ebay_authorize`(第二轮校验补)** —— 见上表:#1 的 `authorization_code` grant 与 #2 的 consent 页原先没有任何工作流认领,与 #31 是同一种缺口;它是**一次性人工链、不进 `schedule.JOBS`**,故**行号计到 14,但业务链仍是 13 条**(`docs/ebay_plan.md` 引的"9 → 13"指的就是这 13 条)。
⚠ **反过来,`docs/ebay_plan.md` 立了而本矩阵没有的两条不是漏收**:`ebay_order_audit`(批次 6,订单风控)与 `ebay_kpi`(批次 9,日报底座)**一个 eBay 端点都不调**(前者只读本库 `orders.order_lines`,后者只汇总本库),按本文"只列工作流会用到的端点"的收录规则不进矩阵。
...
**端点硬要求(全部 verified,同页逐字)**:① 必须用 **https**,路径**不得含内网 IP 或 `localhost`**;② 挑战码握手——端点必须把 `challengeCode` + `verificationToken` + `endpointURL` **hash 到一起并回 `200 OK`**;③ 每条通知**即时**以 `200 OK` / `201 Created` / `202 Accepted` / `204 No Content` 之一应答。④ 走 Notification API,配额桶见 §3.1(10,000/day)。⚠ **交付物(建在哪、谁续证书、挂了怎么发现)不在 api 层,归 `docs/ebay_plan.md`,已立于该文批次 11(3 人日:选型 + 端点实现 + destination/subscription + 每日订阅状态自检并进 `ebay_account_health` + 证书续期责任人写进 runbook);该文判据 ⑦ 剩下的只是"怎么建"那半。**
...
- **Trading Revise Listing 类**(ReviseItem / ReviseFixedPriceItem / ReviseSellingManagerTemplate):**1200 calls / 30 秒**,超限 ErrorCode **21919144** [verified KB2137]。⚠ **这是卖家级不是应用级**:不会被"多开 App"绕过,也不会因店多而稀释。⚠ **Trading 通用超限错误 518 记「⚠ 待复现」**(W4 记 verified,2026-08-25 校验轮未能复现:其自查端点 `GetApiAccessRules` 的参考页现已 404,字段清单亦未取到;与 `refdata/ebay_rate_limits.tsv:87` 同状态)。⚠ **Sell Feed(LMS)的「每卖家 per feedType 400 requests/day、单文件 ≤15 MB、超限 error 160025」三个数同记「⚠ 待复现(源页 `data-file-limits.html` 未复现)」** —— 校验轮试的 `sell/static/feed/feed-limits.html` 返 **404**,官方 api-call-limits 页只给 "Feed API: **100,000** API calls per day"(**不同粒度**,不构成矛盾但也不支持那三个数)。**本项目不用 LMS,低危,留在"登记备查",但任何设计不得依赖这三个数。**
  ⚠ **这里的"待复现"要读准,别读成"未核验",更别读成"官方没有这个数"**:原始抓取(W4)给这三个数登记的来源页是 **`sell/static/feed/data-file-limits.html`**(见 `refdata/ebay_rate_limits.tsv` 该块),而校验轮试的是**另一个文件名** `feed-limits.html`(404)⇒ **两轮试的不是同一页,复核并未真正打到原始来源**。故 tsv 与本文一律记 **「⚠ 待复现」而不是「无出处」**;真要用这三个数,**下一轮直接 fetch `data-file-limits.html`**,一次就能定案。
...
5. **令牌落库共享,不放进程内存**(建议 `ops.ebay_tokens`,🔴 **主键四列 `(account, env, token_kind, scopes)`**,其余列含 access_token / access_expires_at / refresh_token / refresh_expires_at)。落库这件事本身是算出来的不是偏好:本仓一天几十条 workflow 各自启停,令牌只活在进程内 ⇒ 刷新次数 = 进程数 × 账号数,而应用令牌只有 1,000/day 且 App 级共享。⚠ **主键里的 `scopes` 不许省,原稿的三列版是错的**(2026-08-25 校验轮定稿,DDL 草案见 `docs/ebay_plan.md` §3.1 并在那里给了同一条理由):**应用令牌是按 scope 集合铸的**(上面第 3 条),缓存 key 不含 scopes 就会拿一个窄 scope 的令牌去调宽 scope 的端点、**回 403 而不自知**;user 行的 scope 集合固定 ⇒ 天然只有一行,加这一列不影响它。真密钥 client_id / client_secret / RuName 仍**只进 `<DATA_ROOT>/.env`**,仓库只出现变量名;落库令牌属运行态凭据,`docs/db_schema.md` 要标注该表**禁止进任何导出/快照**。
...
7. 🔴 **首次授权的兑换动作必须有可执行物,不能只是一句 SOP**(2026-08-25 校验轮补):§1 #1 明写三个 grant_type 含 `authorization_code`,但原稿 §7 只有 `consent_url()`(拼 URL 不发请求)与 `get_user_token()`(刷新),**缺"拿 code 换 token"这一步** ⇒ 下面第 8 条的运维 SOP 依赖一个不存在的东西。定稿:api 层补 **`exchange_code(account, code) -> tokens`**(§7),它是**全仓唯一一处用 `authorization_code` grant 的调用**,职责只有"换 + 返回",落库与打印交给调用方。⚠ **CLI 入口(`workflows/ebay_authorize.py`,`DANGEROUS=True`)与浏览器步骤的 runbook 归 `docs/ebay_plan.md`,已立于该文批次 3 —— 两者都是该批的显式交付物(含新建 `docs/ebay_runbook.md`,且"文档人眼走一遍"单列成一条验收);矩阵侧对应本文 §2 第 14 行。**
...
| `feed_id` | 🔴 **`feed_id` 这个列名不动**;**另加一个可空普通列 `submission_id`** 承接分阶段回执:上架阶段存 `offerId`,publish 成功后存 `listingId`,发货存 `fulfillmentId` | eBay 的回执 id 分阶段,**存最新一个**。🔴 **这一格已按 `docs/ebay_plan.md` §3.1 / 批次 4a 的定稿改写,推翻原稿"`feed_id` 泛化为 `submission_id`"的读法** —— 那是**全域列改名**:`feed_id` 在 `.py` 里 **183 行 / 17 个文件**,`refdata/schema.sql` 里 11 处(含 `ops.feed_items` 主键 `(feed_id, sku)`),而全仓**没有列改名先例**、`ALTER TABLE … RENAME COLUMN` **不幂等**,`db_init` 第二次跑必炸。⇒ **列名不是语义承诺**:`feed_id` 保持原样,分阶段语义由 `submission_id` 这个**新增可空普通列**承载(**普通列才能改值,主键值不能改**),另写进 `docs/db_schema.md` 与 `services/feed_track.py` 头注。**读本文去写 `RENAME COLUMN` 的,就是这一格原来的错。** |
