...
### 19.4 路由的「三路计划」实跑,以及 LEGACY 行的裁决:删(2026-09-06)

所有者问「三路计划是什么,似乎创建了新东西」。**`blacklist_route` 不是新东西**
(9 月 3 日建,9 月 4 日已跑过一次删 42,113 行);新加的只是 `product_level_keep`
一个函数 + 计划里多一栏。三路:

| | 这一行自己的码 | 这个产品的历史 | 去留 |
|---|---|---|---|
| ① | 永久码(或判不出) | — | 留 |
| ② | 可修复类 | **有**够格拉黑的那条 | 留(产品级救下) |
| ③ | 可修复类 | 没有 | 删 |

②查的是产品事件账本,与 `blacklist_push` 回填**同一份判据**(`_judge_events`)。

`--dry-run` 实跑:**将删 347 / 留下 32,369 / 其中产品级救下 33**。我前一天说的
「剩 37」是 §19.5① 那个合计 bug 报出来的错数。

347 条里 **339 条是 `LEGACY`**(项目初期一次性导入的历史行):
`PT_WRONG 302 / EXPIRED 28 / STAGE 6 / PRICE 3`,`error_reclass` 从
`walmart_error_records` / `walmart_items` 找到了它们当年的沃尔玛全文,按新规
判出非永久。这与前一天「历史导入需要保留」撞上 —— 但那句说的是 `rebuild_asin`
**删了灌不回来**(数据丢失),而路由这里是按全文判定、删前落备份。

**所有者裁决:删。** 判据不因进表路径而异:同一段沃尔玛原文,近期扫进来的判
类目选错就放行,历史导入的判类目选错也放行。判不出的 10,020 条 LEGACY 行照旧留
(查不出理由就继续禁)。路由**不用改**。

⚠ 另一侧同样要收尾:9 月 4 日按行级删掉的行里,按产品历史该拉黑的**现在不在表里
= 上架闸拦不住**。`blacklist_push -p backfill=1` 预览「真跑只会新增 1,798 条」
就是这批,`-p apply=1` 加回去。

### 19.5 三个小修 + 「仍在架」收窄(2026-09-07)

**① `error_reclass` 摘要「会被放行的」合计错了**:字典推导 `{c: n for (_o, c), n
in suspect.items()}` 同一个新码来自不同旧类别时**互相覆盖**而不是相加 ——
302 条 `LEGACY→PT_WRONG` 被 8 条 `FLAGGED→PT_WRONG` 盖掉,报「37 条」,路由实算
347。改成 `_split_suspect` 按码相加,守门测试钉住。**报错的数被人拿去做判断,比
不报还坏。**

**② `blacklist_push` 回填预览里「⚠ blacklist_route 删行用的是行级码 —— 判据没
跟上」**:写于 9 月 4 日上午,当天下午路由就加了产品级判定,这句一直没删,
9 月 6 日所有者看到还以为路由有问题。删掉,换成「路由删行前也按产品级判定救一遍,
两边同一份判据」。

**③ 日常进口盖章**:`record_asins` 写行时填 `taxonomy_code/term/version/src`
(`scan` / `feed`)。不盖的话路由每天报「N 条还没回填」(9 月 6 日 348 条,全是
前一天 `problem_scan` 新进的永久码行,路由本来也会留 —— 排不排除结果一样,但
天天报像有事)。见 db_schema 黑名单表补注。

**「仍在架」收窄(所有者定「改」)**:`audit_listing_conflicts` 视图的 CTE 加
`AND w.published_status = 'PUBLISHED'`。起因是所有者贴的一行处置:

```
审核:审核判拒仍在架:General-Use Products | 问题:This item is unpublished because the End Date has passed…
```

不是数据库坏、不是 bug:两条链同轮各建议一次删除,`dispositions` 按 8 月 24 日
的规则把两个理由拼在一行(`审核:` 后到 ` | ` 前是审核的,`问题:` 后是扫描的)。
但「仍在架」与「已下架」**互相矛盾**,病根是视图里「在架」只判
`missing_since IS NULL`(目录里还见得到),不判 published。收窄后已下架的归问题
链(一律删除),审核链只管真在卖的。覆盖面不变。
