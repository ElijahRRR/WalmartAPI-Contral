...
**台账写"未落定"有三种原因,处置完全不同**,`-p probe=1` 把沃尔玛那一侧并排
摆出来当场分档(判据在 `workflows/feed_poll._verdict`,吃 head 原件):

| 沃尔玛 feed 级 | 台账 | 是哪一档 | 怎么办 |
|---|---|---|---|
| 已终态 | 还有未落定 | **残留**:那几个 SKU 自己的 `ingestionStatus` 不是终态 | 逐 SKU 看是 INPROGRESS 还是枚举没认出来;后者补 `api/feeds._SKU_*` 码表 |
| 非终态,但已给出成功/失败计数 | 一个都没落 | **结论在沃尔玛手上、我们没去取**:`poll_feed` 对非终态 feed 只读 head 不翻明细 | 见下面「已知缺口 ②」|
| 非终态,全部待处理 | 一个都没落 | 沃尔玛**真的还在跑** | 等 |

#### 已知缺口 ②:非终态 feed 的已有结论取不回来(2026-09-11 实数据发现)
...
**没修,待所有者拍板**(与放弃期限同一批):给非终态但超龄的 feed 翻一次明细、
把已有结论的 SKU 落账(仍不 `mark_feed_done`、**不**跑"明细里查无 ⇒ missing"
那一步 —— 那些 SKU 是真的还在处理)。纯增量、幂等、不调任何写接口,但它会让
几千行飞书投影与产品事件提前落定,属于行为变更,不擅自做。
