...
> 与〇节冲突的部分作废(清单见〇.5)。〇节已全部拍板并实施(〇.6 / 〇.7;pending 即 Q3 同日批)。2026-09-26 实际结果判定暂停、新口径待拍板(〇.8)。**
>
...
### 〇.8 实际结果「回看 7 天」复核(2026-09-26,判定已暂停,新口径待拍板)
...
2. **观测前同一参数又改过就不判**:只有最后一次改动能被这次观测证明;被后面改动盖住的那次不判。
   ...
   停用 / 删除)。
3. **库存:这期间有订单就不判**(订单会扣库存,判不准);没有订单时等于目标 = 生效,不等 = 未生效。
