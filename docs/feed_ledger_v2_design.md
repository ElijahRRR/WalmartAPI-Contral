...
> 与〇节冲突的部分作废(清单见〇.5)。〇节已全部拍板并实施(〇.6 / 〇.7;pending 即 Q3 同日批)。2026-09-26 实际结果改口径(〇.8,所有者批,库存改为不判)。**
>
...
### 〇.8 实际结果「回看 7 天」复核(2026-09-26,所有者批新口径,库存改为不判,已实施)

**起因**:上线第二天 06:40 日报链卡死 —— 实际结果候选查询逐条去 `ops.dispositions` 取目标值,
feed_id 上没有索引,1.88 万个候选各把 65.8 万行扫一遍,跑了 44 分钟不报错(库里没设超时)。
性能已修(整批 join + `dispositions_feed_sku_idx` + 本步 5 分钟超时 + 连接缺省 30 分钟超时)。
所有者同时指出口径问题:「完全没有必要 7 天,有的产品同样的参数可能在这七天修改了很多次,
你直接查询可能查不准」。**新口径定下来之前判定暂停**(`feed_effect.PAUSED`):判一次不回头改,
按旧口径判下去的会永久留在表里。

**复核结论(旧口径的四个洞)**:
1. **拿今天的快照比一周前的提交**。catalog_sync 一天两轮(06:40 日报链、13:00 产品链),每个操作
   过了落定期限后的**第一次观测**最迟约 18 小时后就来了;一周前的提交与今天的快照之间隔着十几次
   观测、好几轮维护,今天看到的值早就不是那一次操作的结果。
2. **只排除了同 feedType 的后续 feed**。同一参数还有别的改法没算进去:维护链小批量走单品 PUT
   (处置行 feed_id='sync',不进 feed 台账)、同一参数的另一种 feed(inventory ↔ MP_INVENTORY、
   node_clear 的库存 feed、上架/跟卖/改码的 MP_ITEM / MP_ITEM_MATCH 也带价格和库存)。
3. **库存会被订单扣**。改库存成功后卖出一件,快照就比目标少一 —— 判成「未生效」是误报。
4. (附带)维护三类在 `dispositions.settle_maintenance`、删除在 `product_events.verify_deletions`
   各有一套既有判定,口径与这里不完全一样;本次不合并,只记在这里。

**新口径(提案)**:
1. **只在期限后的第一次观测判**:每轮只判「落定期限落在该店上一次观测与这一次观测之间」的明细;
   错过这一次(店缺席、这轮没扫到这个 SKU)的**不再补判**,摘要记「错过观测 N」。每店上一次观测
   时刻由 catalog_sync 每轮记一笔(`ops.cursors` 一行,不建新表)。回看窗口因此自然只有一轮
   (约半天到一天),7 天的常量删掉。
2. **观测前同一参数又改过就不判**:只有最后一次改动能被这次观测证明;被后面改动盖住的那次不判。
   「改过」= 同店同 SKU、在这次提交之后、这次观测之前,有任何一次动了同一参数的提交:价格
   (price / MP_ITEM / MP_ITEM_MATCH / 单品 PUT)、库存(inventory / MP_INVENTORY / MP_ITEM /
   MP_ITEM_MATCH / 单品 PUT)、标题(MP_MAINTENANCE / MP_ITEM)、在不在架(上架 / 跟卖 / 改码 /
   停用 / 删除)。
3. ~~库存:这期间有订单就不判~~ → **所有者改为「库存不观测结果」:库存一律不判**(inventory /
   MP_INVENTORY 不进候选,库存动作的目标值也不取)。
4. 性能:候选只剩一轮的量(几千条),全部走索引(`dispositions_feed_sku_idx`、`feed_items (store, sku)`、
   `order_lines (sku)`),本步 5 分钟超时照留。
5. 恢复判定从拍板后的下一轮开始;暂停期间与旧 7 天窗口里的存量**不补判**(已经错过第一次观测)。

**拍板**(所有者 2026-09-26):「合并 pr,新口径下,库存不观测结果」⇒ 1、2、4、5 按提案,3 改为库存不判。
**实施**:`services/feed_effect.judge` —— 每店窗口 (上一次观测, 这一次观测],这一次 = 该店在架行最新
`last_seen_at`(与缺席判据同口径),上一次记在 `ops.cursors` 'feed_effect:observed'(与判决同事务);
某店第一次出现只记基线;观测前同参数又改过(`_SAME_PARAM` 的后续 feed、单品 PUT 改价)标 overridden
不判;期限后这一轮没观测到的记 missed 不补判。`feed_effect.PAUSED` 与 `LOOKBACK_DAYS` 删除。沙箱
生产量级(65.8 万处置 / 71.8 万 feed 明细 / 50 万目录,一天的窗口)全部 100 店 0.43 秒,全程走索引。
