...
| 黑名单亚马逊类目 | 黑名单中心·类目维度(审核 Phase0 类目闸数据源;所有者定稿 2026-08-13) | **飞书→PG**:risk_sync 镜像入 catalog.amazon_cat_blacklist(同上 TRUNCATE+双护栏;入库即归一化,与查询侧共用 normalize_amazon_category) | **wiki 承载电子表格**(同一文档,复用 FEISHU_BLACKLIST_WIKI_TOKEN);.env FEISHU_AMZCAT_BLACKLIST_SHEET_ID;单列 A=类目路径(表头条件式同上) |
