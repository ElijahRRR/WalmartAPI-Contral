...
| 上下架限额表 | **按店铺分行**的单日上/下架限额(店铺/fba区间1/fba区间2/FBM区间1/FBM区间2/上架限制/下架限制/库存特殊要求) | 飞书人工维护 → 程序读(daily_retire 读「下架限制」,店铺不在表内退默认值并告警;未来 listing 读「上架限制」) | 多维表格;代码已接入,token 待填 .env(FEISHU_LIMITS_APP_TOKEN / FEISHU_LIMITS_TABLE_ID) |
