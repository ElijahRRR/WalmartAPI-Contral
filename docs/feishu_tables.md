...
| 维护记录 | maintenance 流水账(只追加) | 程序是唯一写入方(**两条链都写**:maintenance 写标题/价格/库存;problem_product_cleanup 自 2026-08-24 起写删除/顽固停用/反补):提交时一次写全 11 列,feed 路径 **H=真 feedid、J=处理中**;PUT 同步路径 **H="sync"、J 当场落定**,**J/K** 由 feed_poll 反哺器(sync_from_ledger)按 ops.feed_items 回填;水位在 ops.cursors('maint_sheet') | **电子表格**:「在线产品总表」spreadsheet 内的「维护记录」工作表(所有者已建 2026-08-07;bitable 5 万行上限装不下);**A~K 共 11 列**(2026-08-16 所有者加「建议」「原因」两列,9→11):店铺/SKU/建议/原因/动作/旧值/新值/feedid/日期/结果/报错;列序唯一权威 = `registry.resources.MAINT_SHEET.columns`,代码一律 `_col(名)`/`_idx(名)` 取位置,不许写死字母;sheet_id 填 .env FEISHU_MAINT_SHEET_ID |
