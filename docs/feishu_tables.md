...
| 产品分配表(新) | **点名分配**驱动表(`alloc_plan -p from_sheet=1`,allocation_plan.md 口径 #19) | 运营只填 **ASIN** 列;其余 16 列脚本一次写满:店铺 / 品牌 / 产品分 / 罚分 / 罚分原因 / 流别(只有 自由流 / 定向流 / 未分配 三值)/ 未分配原因 / 商品品类(五大类) / 商品大类(26类) / 评分 / 评论数 / 配送方式 / 配送天数 / 落地价 / 窗口销售额(毛额) / 是否在线(是/否)。**「店铺」已填的行重跑跳过**(不读不写),所以 dry-run 不回写;**PG(`catalog.claims`)权威**,表是所有者照着去上架的投影(占用 source=`alloc_plan_sheet`)。⚠ 读写**按表头名定位列**(`services/sheet_layout`,与上架表同一份算法):挪列不用改代码,表头缺列/重名 fail-closed 拒绝读写,多出的列只告警;表头文字只在 `registry.resources.ALLOC_SHEET.headers` 出生 | **电子表格**:「产品中心」文件(= 在线产品总表同一文件)内的「产品分配」工作表(所有者建 2026-09-17);token 复用 FEISHU_ONLINE_SHEET_TOKEN,sheet_id 填 .env FEISHU_ALLOC_SHEET_ID |
...
| 上下架限额表 | **按店铺分行**的单日上/下架限额 + 店铺目标 + 渠道限制(店铺/fba区间1/fba区间2/FBM区间1/FBM区间2/上架限制/下架限制/库存特殊要求/**目标销售额/目标订单/单店最大在线数**(2026-08-12 建列,销售额与订单为**日目标**,最大在线数是总容量上限)/**配送限制**(2026-08-13 建列,填 fba/fbm,一店一渠道的权威;**一列三个消费方**:分配未填=不接自由流,上架/维护未填=不限制,见 registry 该列注释)/**类目1/类目2/类目3**(2026-08-15 建列:店铺准入大类目的**唯一权威**,只准入表里填的大类,三列全空 = 该店不限制类目;可填值 = 五大品类 + 「其他」+ 26 个 Walmart Category,大小写与多余空白不算错,认不出的值由 alloc_audit 逐店点名)/**配送时长限制**(2026-08-16 建列,逐店配送天数上限;**一列三个消费方**:分配 `store_targets.lead_ok` 只分 ≤该值 / 上架 list_new 超限不上架 / 维护 maintenance 超限库存写 0;没填时分配=不限,上架维护=回落全局 `amz_source.MAX_LEAD_DAYS`(=7))/**商品上限**(2026-09-07 建列,**沃尔玛给该店的 item setup limit**:店内现有 item 数 + 本 feed 条数 超了它,沃尔玛**整个 feed 拒收**——`feedStatus=ERROR`、`itemsReceived=0`、零逐条明细、feed 级 `EXT_DATA_ERROR_50575703577001`,2026-09-07 A131吕灿荣 整店改码 2740 条实证。填 Seller Center 查到的该店真实上限,**留空 = 走缺省 5000**(`resources.WALMART_ITEM_SETUP_LIMIT_DEFAULT`,出处即那句报错原文);读列唯一口 `services/store_limits.setup_limits`,消费方 = 改码 `sku_migrate` 的上架上限闸(余量 = 本列 − 该店现观测在架 item 数)。⚠ **与「单店最大在线数」是两回事,别合并**:那是我们自己定的经营容量目标(分配引擎读),这是沃尔玛的硬限)/**最大库存**(2026-09-25 建列,**每店单品最大库存 N**,留空或 0 = 不限;门槛(全局 5)决定卖不卖:亚马逊库存 <5 上架不上、维护写 0;N 决定卖多少:过了门槛写 min(亚马逊库存, N)。**一列两个消费方**:上架 `list_new`(上架数量)、维护 `maintenance_intents`(跟随亚马逊那一支);读列唯一口 `services/store_limits.stock_caps`,换算唯一实现 `store_limits.stock_for`。跟卖铺货、stockzero、非 amz 来源行不受管。⚠ **不是「库存特殊要求」**:那一格的 0 是整店停售开关)) | 飞书人工维护 → 程序读(product_clear 读「下架限制」;list_new 读「上架限制」;分配引擎读目标三列与配送限制;**上架 list_new 与维护 maintenance_intents 自 2026-08-25 起也读「配送限制」**,取数唯一口 services/store_targets.store_channels) | 多维表格;代码已接入,**token 已配(2026-08-17)**。⚠ 没配时最要紧的两处是静默降级:倍率读不到 ⇒ 区间内产品定不出价、只有出界品能上(300% 兜底);`库存特殊要求=0` 名单为空 ⇒ 该清零的店一件没清 |
...
- **销售订单(审核列)**:同一张表的第二个登记条目 `ORDER_SALES_AUDIT`,
  ...
  卖家店铺名、产品截图、**建议采购方**、限价、**标题相似度**。**不碰**:建议采购日期
  (人工域,所有者定稿 2026-08-09 明确排除)、**采购方**(2026-09-07 起归人工填
  实际采购方;脚本判出的那个改写「建议采购方」列,所有者新建后切换)。
