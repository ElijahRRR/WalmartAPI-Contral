...
| 上架表(新) | listing 主驱动表(L2) | 运营只填 **A=店铺 B=ASIN**(2026-08-16 所有者对调了这两列;此前 A=ASIN B=店铺),运营还填 **C=SKU**;**D~I 归审核链**(`product_audit -p from_sheet=1` 投影:D 标题 E PT F 审核结果 G 类别 H 具体内容 I 日期),J~S 机器列由 list_new/反哺器写,**T=登记日期 U=查询编码 运营域脚本不写**;21 列 A~U(2026-09-02 所有者改表头:C 插入 SKU、「审核理由」拆成 类别/具体内容、尾部四列 真实标题/PT/UPC/UPC匹配 换成 登记日期/查询编码;此前较旧 26 列砍 状态跟踪/最近跟踪日期)。⚠ 列序唯一出处 = `registry.resources.LISTING_SHEET.columns`,读取按字段名不按字母 | **电子表格**:「在线产品总表」内工作表(所有者建 2026-08-07);sheet_id 填 .env FEISHU_LISTING_SHEET_ID |
