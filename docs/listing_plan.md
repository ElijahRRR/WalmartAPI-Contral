...
- `AMZ_IN_STOCK_QTY`:仅在 `stock_count` 采不到时用;**终值=10**(所有者拍板 2026-08-12)。
  **只限没设「最大库存」的店**(2026-09-25):设了 N 的店,没采到数量不算达到 N,不上架
- 限额表「最大库存」N(所有者定稿 2026-09-25):门槛 5 在前不被替代,亚马逊 ≥N 按 N
  上架、<N 不上架(理由「亚马逊库存不足本店最大库存N」);<5 理由「亚马逊库存不足5」。
  换算唯一实现 `store_limits.stock_for`,与维护链同一个函数
