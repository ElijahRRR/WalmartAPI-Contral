...
4. `python cli.py maintenance_scan -p preview=1` 确认该店意图带 ship_node、
   校验通过;
5. 之后新上架自动进指定仓;
6. **存量 SKU 恢复维护**(如需):Seller Center → Shipping template →
   Assign SKUs 把存量 SKU 关联到新 FC(可分批)→ 下轮 catalog_sync 落节点行
   → 维护链自动接管。代码侧**不做**自动搬仓(所有者拍板 2026-08-30);
   未搬的存量行库存维护暂停,见 §6 末段。
...
**存量行路由(所有者拍板 2026-08-30,谭总12 实测后二选一定稿)**:配置店
**只维护受管仓**,不做"按 SKU 所在仓路由"。后果全部知情接受:
- 存量 SKU(还在 Virtual Node、未与新仓建立关联)的**库存维护整体暂停**
  (清零/跟库存/跟卖铺货三路都跳过并计数),直到搬仓;断货不清零的超卖
  风险由所有者自担。价格/标题/删除不受影响(与仓库无关)。
- "跳过计数长期非 0" 对未搬仓的存量行是**预期状态**,不是故障;搬完仓
  这个数还不掉才查 §2.4。
- ⚠ 跟卖铺货的"未知 → 保守铺上"旧口径对配置店**反向了**:配置店的未知是
  "SKU 未与新仓关联",当没货铺会往新仓写保守值而存量货还在 Virtual Node,
  两节点同时有货 —— 已改为与 inventory_intents 同一条 fail-closed 口径。
- **搬仓的机械**(实测 2026-08-30 得到的关键事实):节点行出现在
  `GET /v3/inventories` 的门槛是 **SKU×FC 关联建立**(Seller Center →
  Shipping template → Assign SKUs),**不要求有货**(qty 0 也出行)。
  所以存量恢复维护的路径是:给存量 SKU 建关联 → 下一轮 catalog_sync 落
  节点行(qty 0)→ 维护链自动接管(该清零清零、该跟库存跟库存,写的都是
  新仓)。不需要人工先写库存。
