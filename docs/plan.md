...
| 10 | listing | auto_listing + match_listing | **是** | **[~] L1 跟卖生产验收通过**(2026-08-07,所有者确认:A107 单店试点全链路;对拍定稿 SKU 人工优先/留空自动续号;来源登记簿 catalog.listing_sources 同批落地)。子计划见 docs/listing_plan.md:L0 地基(部分)/L1 ✓ → L2 上架主链(UPC 池专题讨论先行)→ L3 自愈链 → L4 收尾。**前端不迁移**(所有者定稿;server 职能由 PG 事务替代);价格/库存同步归 maintenance provider;spec 文件入 `<DATA_ROOT>/specs/<版本>/` |
