...
### 2026-09-25 每店单品最大库存 + 维护补回门槛 5 + node_clear 自动发现旧仓(所有者定稿)
...
`node_clear` 改为**自动发现旧仓**(`GET /v3/inventories` 逐 SKU 的 `shipNode`):
填了「维护仓库」且校验通过的店,受管仓以外有货的节点全清;不带 `-p store` 即
所有配置店一起跑,店间并发、失败店串行补试一遍。「只清受管仓已接管的 SKU」
「拒绝清受管仓」两道闸照旧;接口没给 shipNode 的数量不碰(不带节点只能写默认
节点,可能正是受管仓)。
