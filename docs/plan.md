...
| — | variant_offset_cleanup | (新增) | **是** | 采集永久偏移的 SKU 删除(所有者定稿 2026-08-09:「对于这种 variant_offset 的 sku 做删除动作」)。`variant_offset` = 亚马逊把 /dp/<ASIN> 返回成兄弟变体页面,parser 比对页面 ASIN 不一致后拒绝写入,采集侧列为**不自动重试**类型 ⇒ 这些 SKU 的价格/库存永远拿不到新数据,维护链只会拿陈旧快照跟价跟库存。两道保守闸:①`min_batches` 默认 2(≥2 个批次都偏移过才删,`-p min_batches=1` 覆盖)②最后一次偏移之后若有 `outcome=ok` 快照则自动移出名单。单店上限 300。**[~] 代码就绪**(2026-08-09,首轮实测 149 个 ASIN 全 variant_offset):待生产验收 |
