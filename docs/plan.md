...
7. **遗留待议**:`currently under review`(序 16 显式 OTHER,自愈态)未列入可恢复码,
   现按「其他」删;RETIRED 行带政策原子被建议删除后若回执为「已停用」类失败,先从
   `ops.feed_items` 拿到实际错误码再决定要不要加 WFS 同款的最近回执闸(不按记忆
   编码);审核链为什么放行 Plant Seeds(`audit_why -p asins=… -p runs=3`)与
   类目表要不要给 Plants & Seeds 之类政策加类型级闸。
