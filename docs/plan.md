...
| 7 | product_clear | 沃尔玛批量下架(旧 daily_retire) | **是** | **[~] 代码就绪**(2026-08-06):飞书停用/删除表驱动(A~D 运营填,E~G 程序写),动作列路由 停用/下架→RETIRE_ITEM、删除→DELETE_ITEM;状态权威改 ops.feed_log(旧"只存飞书三列"风险消除);失败行不自动重试(清 E 列重排队)。待:用户建表配 token、限额表口径定稿、dry-run 生产验证。⚠切换前停旧 15:00 cron。命名原则(所有者 2026-08-06):新工作流按功能命名,不继承旧系统名 |
