...
| — | product_ingest | (新增) | 否 | 采集服务(amazon-scraper-v4)增量 → 产品中心 catalog.products/snapshots。**全项目唯一直连采集器的工作流**(漏斗铁律);契约 v1 + §5.1 补遗(409 硬停/三值 stock_state/slow_hash 不透明/空值不覆盖/404 非空数据);游标 ops.cursors,空页不推进。**[~] 代码就绪**(2026-08-08):待本机采集服务起服务后端到端验;⚠ 契约无数值库存,IN_STOCK_QTY 保守常量待所有者定夺 |
