...
| — | cleanup_history_import | (新增,一次性) | 否 | 旧问题商品三笔历史(error_items 41.7 万行时间线折叠 → product_events;seen 20.1 万对 → ops.cleanup_seen_categories;brand_cache → ops.dedupe)。**[~] 骨架就绪**(2026-08-11,建模按所有者定稿走时间线;缺产品行不阻塞):预览模式先探测旧表列名/JSON 形态再 apply,事件侧擦净重灌幂等。待:生产 Mac 实跑(旧库 peer 认证本机连,LEGACY_CLEANUP_DSN 可覆盖) |
