...
| 6 | maintenance | 沃尔玛商品维护 | **是** | 含清库存;maintenance.db 数据并入 PG listing schema。**维护事件入账定稿**(所有者 2026-08-07):标题/价格/库存维护(含清库存)一律**不进** catalog.product_events——清库存是店铺维度运营操作,系统不设店铺维度病历;流水在 ops.feed_log/feed_items,现状在 catalog.walmart_items,状态后果由 catalog_sync status_changed 观测入账。配套闸已落地:feed_track 回执入账白名单(receipt_in_ledger)+ 反补计数 source 过滤 |
