...
| **listing / list_new** | ✅ **端到端 2026-08-13**(3/3 SUCCESS,六轮错误账收官) | **变体分组(功能余项)**;挂调度+停旧切换 |
...
| **backup** | ✅ 代码完(2026-08-13 批次 A:-Fc 原子落地 + pg_restore 校验 + 保留期清理) | 生产首跑;挂每日调度 |
...
| — | audit_import | (新增,一次性) | **是** | 旧审核库 walmart_audit(同实例隔壁 database)13 表 → audit schema。dry-run 逐表体检(pg_attribute/format_type 含精度列对照;源独有列/类型不符/清单表缺失=致命拒迁;replace 的 TRUNCATE 必预告);--execute 单事务 COPY + 行数校验 + 标识列 setval;源连接 REPEATABLE READ 只读。**[~] 代码完成**(2026-08-13 批次 A);前置:生产 `pg_dump -s walmart_audit` 核对三张反推表 DDL + 停旧审核写入方后执行 |
