...
| 10 | listing | auto_listing + match_listing | **是** | **[~] 子计划立案**(2026-08-07,见 docs/listing_plan.md):L0 地基 → L1 match_listing 先行 → L2 上架主链(采集/xlsx 双 provider,UPC 池 PG 化切换点)→ L3 自愈链 → L4 收尾。**前端不迁移**(所有者定稿:erp_listing_server/erp_web/erp_worker 不搬,基础脚本迁完后统一重构前端;server 的 UPC 分配/去重 cache 由 PG 事务替代);价格/库存同步归 maintenance provider 不在 listing 重做;spec 文件入 `<DATA_ROOT>/specs/<版本>/` |
