...
设了 N 的店:亚马逊库存 ≥N 写 N、<N 写 0。留空或 0 = 不限。

**定稿八条**(所有者逐条拍板):
1. 新列「最大库存」,读列唯一口 `store_limits.stock_caps`。
2. 公式确认是「≥N 写 N、<N 写 0」,不是取两者较小值(N=20、亚马逊 12 → 0)。
3. **门槛 5 不被 N 替代**:门槛确保亚马逊库存是多的,N 确保卖的数量不会太多。
   **维护也补回门槛 5**:亚马逊 <5 写 0,**所有店**(此前 1~4 件照写原数,
   是旧系统「同步时 <5 推 0」没迁过来的漏项)。
4. 上架算出 0 = **不上架**,理由写判出 0 的那道闸的值(「亚马逊库存不足5」/
   「亚马逊库存不足本店最大库存N」;门槛以后可能调,数字跟常量走)。
5. **没采到数量不算达到 N**:设了 N 的店,「页面显示有货但没数量」不再按
...
预览按店摊开;上架闸门行加「低于本店最大库存 N」「按本店最大库存上架 N 行」。
