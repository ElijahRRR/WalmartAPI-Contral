...
### 2026-09-25 feed 结果按官方期限收口 + 实际结果分表 + 观测驱动的自动重做改交人工(已实施,pending 待定)
...
**pending 未定**:所有者问「说明可能没提交上?现在的逻辑是什么样子的」,已答(pending ≠ 没提交上;
现行只告警、永不老化),建议仍是只读反查 + 到期落 failed「提交未确认」+ 不自动补交,等所有者定。
