...
> | | 数量 | 现状 |
> |---|---|---|
> | 电脑 launchd | 4 条 | `feed_poll`(每半小时)、`order_chain`(每小时 :20)—— 已装载,首跑 exit 0;`product_ingest`(每小时 :50,2026-08-19 追加);`store_watch`(每小时 :45,2026-08-30 追加,**上线三步见 `docs/store_events.md`** —— 装调度之前必须先手动 `-p seed=1`) |
...
| 每日 05:00 | `item_id_sync` | 用 On-request ITEM 报表补 `walmart_items.item_id`(2026-09-07 所有者定稿):只为有缺口的店各拿一份,创建每店每小时一次、等 ≤60 分钟,限流/超时下轮接着;首轮手动 `-p all=1`;飞书 itemId 列由 06:40 的 catalog_sync 投影 |
| 每小时 :50 | `product_ingest -p lock_wait=900` | 全局增量泵(2026-08-19 起单独长驻 launchd):本地产品中心 ↔ 采集器数据库对齐,各链按批自取之外的全部增量走这条 |
| 每小时 :45 | `store_watch` | 店铺事件账本(`ops.store_events`)的**唯一推送出口**(2026-08-30):扫未推送高危 → 一轮一条飞书 → 标已推,顺带比对治理配置快照。:45 是为了避开另外三条的分钟(:00/:30、:20、:50)——锁按工作流名互不相干,但会一起挤代理与 PG 连接。⚠ 装调度之前必须先手动 `-p seed=1` 吞掉存量,见 `docs/store_events.md` |
