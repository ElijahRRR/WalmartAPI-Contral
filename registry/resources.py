...
# ⚠ **下面这组数是「Flash 系列新价 = V4.1 Flash 单价」**,也正是本仓实际被扣的钱:
#   缺省模型 deepseek-v4-pro 在路由期跑的就是 V4.1 Flash、按它计费(见
#   LLM_MODEL_ALIASES)。所以这一行不因"官方文档页什么时候更新"而动。
# ⚠ **官方文档页滞后,已复核两次**:9/10 11:13(生效前 47 分钟)与 **12:17
#   (生效后 17 分钟)** 两次读定价页都仍挂旧价(v4-flash 空闲 0.05 / 1.5 / 4.5,
#   那是 8/17 那次调价后的值);带查询串强刷同样是旧价,更新日志最新一条仍是
#   8/21 —— 是文档站滞后,不是缓存。新价与路由由官方公告 + 多方报道共同印证。
# ⚠ **留一个口子**:若日后确认 **V4-Flash(非 4.1)本身没跟着降**,而又有人把
#   .env 显式设回 `deepseek-v4-flash`,要给它**单开一行旧价**(空闲
#   0.05 / 1.5 / 4.5、高峰翻倍)—— 别直接改下面这一行,那会把我们真实在付的
#   V4.1 Flash 单价一起改错。宁可估贵不估便宜(与 llm_cost 同一条纪律)。
