...
WITH due AS (
    ...
    WHERE f.status = ANY(%(judgeable)s::text[])
      ...
      -- 已过落定期限(未到期的不进后面的 LATERAL,判也判不了)
...
LEFT JOIN LATERAL (
    SELECT x.action, x.detail FROM ops.dispositions x
    WHERE x.feed_id = d.feed_id AND x.sku = d.sku
      AND x.action = ANY(%(maint_actions)s::text[])
    ORDER BY x.id DESC LIMIT 1
) disp ON true
LEFT JOIN LATERAL (
    SELECT ni.avail_qty FROM catalog.item_node_inventory ni
    WHERE ni.store = d.store AND ni.sku = d.sku
      AND ni.ship_node = disp.detail ->> 'ship_node'
      AND ni.seen_at > d.due_at
    LIMIT 1
) nq ON true
"""
