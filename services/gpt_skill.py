...
def skill_md() -> str:
    ...
    lines = [
        ...
        *_table(),
        ...
        "```",
        "",
        # ⚠ 条数**必须跟着调度表算**:写死"这两条"的那版在 launchd 侧加到第
        # 三条时就已经对不上了(2026-08-30 发现),而生成的提示词里一句错话
        # 不会有任何东西报错 —— 智能体照着"两条"去数,第三条自然被当成多余的
        f"## 这 {len(launchd_jobs)} 条不进你的定时任务表,但**得有人管**",
        "",
        "| 任务 | 时间 | 跑什么 |",
        "|---|---|---|",
        *[f"| `{j['label']}` | {when(j)} | {' → '.join(j['workflows'])} |"
        ...
        "",
        "它们跑在**电脑自己的 launchd** 上,而不是你的定时任务里 —— 频率太细"
        "(每半小时 / 每小时固定分钟),你那边多半排不准,而排不准的后果不是报错,"
        "是悄悄少跑几轮。",
        "",
        "⚠ **别把这一节读成「不用管」。** 装 launchd 那一步本身**是要人做的**,"
        f"做法在 `{SKILL_DIR}/REGISTER.md` 第 3 步(`launchd_install` → "
        "`launchctl load` → 回读校验)。**没装 = 这几条链从来不跑,而且没有任何"
        "东西会说一声** —— 表现是飞书上的「处理中」永远不消失(feed 回执没人反哺)、"
        "日报的订单列是空的(订单没人拉),而所有已注册的任务都报成功。"
        "拿不准装没装就去查:`launchctl list | grep "
        + schedule.LABEL_PREFIX.rstrip(".") + "`,应当正好 "
        + str(len(launchd_jobs)) + " 行。",
        "",
        "⚠ **装好之后,你这边不要再挂一份 —— 两边都挂 = 撞锁**:同一条链被 "
        "launchd 和你同时拉起来,后到的那次拿不到锁直接退出码 3 空跑一轮"
        " —— 看起来一切正常。",
        "",
        "## 三条通用规则(每条任务都适用)",
        "",
...
def register_md() -> str:
    ...
    lines = [
        ...
        *[f"| `{j['label']}` | {when(j)} | `{cron(j)}` | `{cron_utc(j)}` | "
        ...
        "按这行切开即可。)",
        "",
        f"## 第 3 步:这 {len(schedule.jobs_for('launchd'))} 条"
        f"**不要注册成你的定时任务**,改用电脑的 launchd",
        "",
