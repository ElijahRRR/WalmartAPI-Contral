...
6. **要人动手的才逐条,其余归并成一行**(2026-09-10 补,实证代价见下)。
   判据是"读的人要不要动手",不是"行数多不多":`⚠ 撞车、请人工处置` 这种一条
   都不能省;而"设计好的后果、每条都对"那种(改码链的「维护账随码迁」)一行说全
   计数 + 分档 + 几个样本即可。⚠ 这不是化妆:A131吕灿荣 一轮 settle_only 把几百条
   信息各写一行,**飞书整条通知被拒**(`code=230025` 消息体超长),那轮摘要一个字
   都没推出去,而真正要人看的两条 ⚠ 就夹在那几百行里。`api/feishu.notify` 的长度
   闸只是保险丝(超长保首行 + 截断说明 + warning),主防线是这一条。
