...
区间定稿(表格中不可见,所有者口述定稿,勿改):
  FBA:区间1 = 0~30,区间2 = 30~1000(上界 2026-08-21 由 75 抬到 1000)
...
PRICE_BANDS = {
    # ⚠ FBA 区间2 上界 2026-08-21 由 75 抬到 1000(所有者)。副作用要知道:
    # 75~1000 这一段此前**出界走 300% 兜底**,现在改吃限额表的 fba区间2 倍率
    # ——该店那一格没填的话,这段价格从"照样出价"变成"不定价"(上架不上、
    # 维护不改),这是配置缺失的正确表现,但换版当天会看见一批新的跳过。
    "FBA": [(0, 30, "fba_range1"), (30, 1000, "fba_range2")],
