...
#:     中位数 38.4 掉到线下,可分配从 249,449(线在 35)缩到 180,057。
#: 改 `BASE_MAX` / `SALES_BONUS_MAX` 时**必须回头看这一行**。
#:
#: 同一天走了个来回,两次都是所有者定的,理由不同:
#:   40 → 35(怕牌堆撑不满 `可分 × 1.5` 的切口,分配质量落到淘汰线);
#:   35 → 40(**「反正现在产品用不完」**)—— 实测切口只吃掉牌堆的一小半,
#:   收紧之后余量仍然够,那就该让线回到"扫烂品"的本意上。
#: ⚠ 40 **高于**「中等口碑(4.2 分 / 50 评)无销量」那档的 37.7,也就是说它
#:   已经不只是扫烂品、而是砍掉了一半可判分的货。这是**明知故为**:货多得
#:   用不完时,宁可只在好的一半里挑。回归 `test_cutoff_...` 钉的是**比例**,
#:   不是这个具体的数 —— 别把它读成"线可以随便往上抬"。
CUTOFF = 40.0
...
#: ⚠ **2026-08-21 起,配送慢的罚分在分配链上打不到了**:所有者拍 Q3 之后
#: `gate()` 在池口就把 `lead > MAX_LEAD_DAYS` 的货挡掉,而免罚线恰好等于
#: 同一个数 ⇒ 进得了池的货一律 ≤ 免罚线,罚分恒 0。
#: **代码有意保留**,不删:它是闸万一放宽(或将来有别的调用方)时的兜底,
#: 删了就是个静默的洞。`tests/test_product_score` 钉死了这个关系 —— 哪天
#: 免罚线与闸值不再相等,那条测试会提醒有人在改语义而不是改数字。
#: 顺带一提:要让「快货」在分配里真正占优,该做的是把免罚线**降到 7 以下**
#: (比如 ≤2 天不扣、2~7 天线性扣),那是所有者没点头的打分改动,别顺手做。
...
def gate(row: dict, min_stock: int, in_stock_qty: int) -> str | None:
    ...
    **硬闸与分数分开**:这里过不了的根本不参与打分(§7.6)。口径与 list_new
    ...
    ★ **货期也在这里挡**(所有者 2026-08-21 拍 Q3:「不满足选品条件的,直接
    不进入分配」)。上移到池口有两个连带好处,都不是顺手的:
      1. §7.2「组货期 = 组内最长」那条彻底不咬人了 —— 池里没有慢件,组里
         也就不可能有,一件 12 天的货再也带不走整个品牌组;
      2. 「超期」与「没采到」**分成两个原因**报。前者要放宽阈值或换货源,
         后者**补一次采集就能进池** —— 处置完全不同,合成一个数就等于把
         能救的那批藏起来了(自由流的归因刚栽过同一个跟头,见 alloc_engine)。
    ⚠ 阈值直接取 `amz_source.MAX_LEAD_DAYS`,**不开参数** —— 开了就有人忘了
    传,而忘传的后果是慢货静默入池。和上架链共用同一个常量是所有者的要求
    (「和上架那边的要求相互关联」),两边不许各写一个数。
    ⚠ 与 `store_targets.lead_ok` 不冲突:那是**逐店**可以填得更严的一列
    (填 5 天的店仍然只收 5 天内的),这里是全局地板。
    """
    ...
    lead = row.get("lead")
    if lead is None:
        return "配送天数未采到(补一次采集就能进池)"
    if int(lead) > amz_source.MAX_LEAD_DAYS:
        return f"配送超期({lead} > {amz_source.MAX_LEAD_DAYS} 天)"
    return None
