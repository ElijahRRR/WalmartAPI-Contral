...
#: ⚠ 罚分能不能打到,取决于**池口放进来的货有多慢**,而那是逐店条件决定的
#: (`alloc_plan._pool_reach` 取各店「配送时长限制」的并集)。全店都填 7 时
#: 池里没有 >7 的货 ⇒ 这条罚分恒 0;有店填 10 时它就活了。**代码不删**。
#: 要让「快货」在分配里真正占优,该做的是把免罚线降到 7 以下(比如 ≤2 天不扣、
#: 2~7 天线性扣),那是所有者没点头的打分改动,别顺手做。
...
def gate(row: dict, min_stock: int, in_stock_qty: int) -> str | None:
    ...
    **硬闸与分数分开**:这里过不了的根本不参与打分(§7.6)。口径与 list_new
    ...
    ⚠ **货期不在这里判**(2026-08-21 一度写在这里,当天撤回)。它是**逐店**
    的条件:限额表「配送时长限制」一店一填,权威在表里,不是一个全局常量。
    写死 7 会两头错 —— 有店填 10 就白白丢掉它能卖的货,全店都填 5 又让 6~7 天
    的货空占池子。落点在 `alloc_plan._pool_reach`(取所有参与分配的店的**并集**)。
    """
