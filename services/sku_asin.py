...
# 8~14 位)- 价格(可小数,可前导零)
# ⚠ 分隔符收 1~2 个横杠(2026-09-03 生产实证):待归类清单的「其他」桶里
# 有 `CMSQ--B07J2QNQCF-43` 这一支(前缀与码之间两个横杠,疑似旧系统拼串时
# 有个空段)。中段**两侧都有分隔符**,置信度与单横杠的三段式完全相同 ——
# 这一档是**提取**不是猜测,不走 propose_source_key 的 guess 分档。
# 只放宽这一个分隔符:中段本身仍必须字母开头、8~14 位,尾段仍必须是价格。
_WRAPPED = re.compile(r"^[A-Z][A-Z0-9]{0,7}-{1,2}([A-Z][A-Z0-9]{7,13})-\d+(?:\.\d+)?$")
