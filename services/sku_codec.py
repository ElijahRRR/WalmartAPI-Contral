...
#: ⚠ 这里曾有 `MAX_SKU_GENERATIONS = 3`(换码代际上限闸)。**所有者 2026-09-06
#: 决定删除**:「上架表是不断在更新的,没必要设这个上限;上不去的根源是上架
#: 方法的问题,拿上架失败的案例和 feed 返回的具体原因去优化上架才是标准做法,
#: 只防止无限上架治标不治本。」代价是**反复 SKU_LOCKED 的品每个冷却期烧一个
#: UPC** —— 兜底改由退役冷却闸(上面那个常量)限速,不再有终局的人工闸。
#: 别再加回来:要限的是失败原因,不是代数。
