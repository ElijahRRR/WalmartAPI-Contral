...
- spawn **交给 launchd**:`launchctl kickstart -k gui/<uid>/com.walmartapi.yingdao`
  ...
  ✅ **2026-09-01 生产验证通过**:装上代理后日报摘要从「spawn 失败/未配置,
  沿用旧值」变成「抓取新鲜,回填 33 店」。这条路走了三轮才对 ——
  `open` 退 1(08-24)→ 改直启、崩在 _RegisterApplication(09-01)→ 交给
  launchd。前两轮都是**按推断改**,第三轮是按崩溃栈改;教训写在这里:
  同一个症状连续两次"修了没好",要先拿证据再动手,别接着猜
