...
`feed_poll`, `order_chain` 是高频链,已经挂在苏里电脑的 launchd 上。
**你这边再挂一份 = 两边同时拉同一条链**,后到的那次拿不到锁直接退出码 3 空跑,而外表看起来一切正常。

同样地:**上面表里没有的,一条都不要注册**。想加是改 `registry/schedule.py` 再重新生成这份交办单,不是在你这边手动加一条。
