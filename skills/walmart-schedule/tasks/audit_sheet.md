...
备注:审上架表里「审核结果」为空(或 pending)的行 + 把库里已有结论投影回审核六列(2026-09-10 起整表对账:表上结论与库不一致的行也改写)。⚠ from_sheet **不是强审**:已有结论的零 LLM 直接投影,只有未审/pending 过退避的才真判;**缺省不限量**(2026-09-03 定稿:要限量才手动带参数),要分批就改调度表加 -p limit=N,别在提示词里手改。补采(2026-09-10 定稿):领到待审 ASIN 后先看重审开关 —— 重审关(缺省)推「库里没结论且 12 小时内没快照」的,重审开(-p force=1)推「12 小时内没快照」的全部;批次名 audit_gap_<时间戳>(按时间不按日界,手动跑不漏品)→ 插队 → 轮询等采完(缺省 60 分钟)→ 就地摄取 → 采回来的这一轮就判掉;没等到的:在库的用现值审,审不了的把真实原因写进「具体内容」列(「审核结果」留空 ⇒ 下轮重领)。⚠ 时间账:这条链可能跑一个多小时。锁按工作流名各一把,审核拖过 20:00 **不会**让上架退 3 —— 20:00 的 list_new 照常起跑,把还没判完的行按「审核未过:未审」跳过、次日续;待审量大的日子给 -p limit=N 或把上架往后挪
