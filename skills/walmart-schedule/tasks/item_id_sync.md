# 沃尔玛定时任务:item_id_sync(每天 05:00,台北时间)

在 `/Users/nextderboy/Projects/WalmartAPI-Contral` 下执行这一行,**原样执行,不要改任何参数**:

```bash
/Users/nextderboy/Projects/WalmartAPI-Contral/.venv/bin/python3 /Users/nextderboy/Projects/WalmartAPI-Contral/cli.py item_id_sync
```
...
| 步 | 工作流 | 这一步干什么 |
|---|---|---|
| 1 | `item_id_sync` | 用沃尔玛 ITEM 报表补齐 catalog.walmart_items.item_id(首轮全量,日常按缺口)。 |

备注:每天 05:00;有缺口的店各拿一份 ITEM 报表补 item_id(等 ≤60 分钟,限流/超时不是失败,下轮再来);首轮手动 `-p all=1`;飞书 itemId 列由 06:40 的 catalog_sync 投影
