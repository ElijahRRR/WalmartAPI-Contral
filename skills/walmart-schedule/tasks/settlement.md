# 沃尔玛定时任务:settlement(每周三 08:00,台北时间)

在 `/Users/nextderboy/Projects/WalmartAPI-Contral` 下执行这一行,**原样执行,不要改任何参数**:

```bash
/Users/nextderboy/Projects/WalmartAPI-Contral/.venv/bin/python3 /Users/nextderboy/Projects/WalmartAPI-Contral/cli.py settlement_sync
```

这条链跑的是:settlement_sync。

## 这条链在做什么

| 步 | 工作流 | 这一步干什么 |
|---|---|---|
| 1 | `settlement_sync` | 沃尔玛对账明细(从 daily_report 摘出,独立成流)。 |

备注:每周三;账期双周发布,没有新账期那轮自然空转(已入库账期永不重拉)
