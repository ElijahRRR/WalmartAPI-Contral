...
def _wire(monkeypatch, intents, stores=(STORE,)):
    ...
    monkeypatch.setattr(maint_sheet, "append_records",
                        lambda rows: (calls["sheet"].extend(rows),
                                      len(rows))[1])
    # 执行件 2026-08-17 起走 fill_submitted(就地补齐扫描件写的半行)。
    # 桩仍收整行 —— 这些用例断言的是"执行件造出来的行长什么样",
    # 与它是补进已有行还是另起一行无关。
    monkeypatch.setattr(maint_sheet, "fill_submitted",
                        lambda rows: (calls["sheet"].extend(rows),
                                      (len(rows), 0))[1])
...
# ── 扫描件写半行 / 执行件就地补齐(所有者定稿 2026-08-17)────────────────────
#
#   maintenance_scan → 店铺 SKU 建议 原因 日期        (append)
#   maintenance      → 动作 旧值 新值 feedid         (找到那行就地补齐)
#   feed_poll        → 结果 报错                     (不变)

def _sheet_env(monkeypatch, existing, cursor):
    """表桩:existing 是已有数据行(行号从 2 起),返回收集到的写操作。"""
    monkeypatch.setattr(resources, "MAINT_SHEET",
                        Spreadsheet(name="维护记录", token="TOK", sheet_id="SID",
                                    columns=resources.MAINT_SHEET.columns))
    conn = _Conn()
    ...
    _fake_db(monkeypatch, conn)
    monkeypatch.setattr(maint_sheet.feishu, "sheet_values",
                        lambda sheet, rng: list(existing))
    wrote = []
    monkeypatch.setattr(maint_sheet.feishu, "sheet_write_ranges",
                        lambda sheet, ranges: wrote.extend(ranges))
    monkeypatch.setattr(maint_sheet.feishu, "sheet_ensure_rows",
                        lambda sheet, n: None)
    monkeypatch.setattr(maint_sheet, "_find_next_empty", lambda start: start)
    return conn, wrote


def test_fill_submitted_never_overwrites_the_date_column(monkeypatch):
    """回填必须**跳过**「日期」列 —— 它夹在 feedid 和 结果 之间。

    动作/旧值/新值/feedid 是 E:H,结果/报错 是 J:K,中间的 I 是扫描件写的日期。
    当成一段连续区间写,6 个值摊进 7 个格子:整体错位,日期被写成「结果」。
    日期一没,prune 就再也裁不掉这行(它按日期判年龄),而两边都不报错。
    """
    existing = [_sheet_row("T1", "B0A", "", "", "", "", "2026-08-17", "",
                           suggestion="价格", reason="亚马逊涨价")]
    conn, wrote = _sheet_env(monkeypatch, existing,
                             {"next_row": 3, "unresolved_from": 3})
    row = _sheet_row("T1", "B0A", "价格", "10", "12.5", "F9", "2026-08-17", "",
                     suggestion="价格", reason="亚马逊涨价")
    filled, added = maint_sheet.fill_submitted([row])
    assert (filled, added) == (1, 0)
    rngs = [r for r, _ in wrote]
    assert rngs == ["E2:H2"], f"应只写动作..feedid 一段,实际 {rngs}"
    assert wrote[0][1] == [["价格", "10", "12.5", "F9"]]
    assert not any(r.startswith("I") for r in rngs), "碰到日期列了"


def test_fill_submitted_pulls_watermark_back(monkeypatch):
    """补进 feedid 之后必须把水位拉回去,否则反哺器再也不回头看这行。

    sync_from_ledger 会跨过没有 feedid 的行(扫描件写的半行正是这种),
    等执行件把 feedid 填进去时 unresolved_from 早已越过它 ⇒
    结果/报错两列永远空着,而且不报错。
    """
    existing = [_sheet_row("T1", "B0A", "", "", "", "", "2026-08-17", "",
                           suggestion="价格")]
    conn, _ = _sheet_env(monkeypatch, existing,
                         {"next_row": 3, "unresolved_from": 3})   # 已越过第 2 行
    maint_sheet.fill_submitted([
        _sheet_row("T1", "B0A", "价格", "10", "12.5", "F9", "2026-08-17", "",
                   suggestion="价格")])
    saved = [a for sql, a in conn.sqls if "ops.cursors" in sql][-1]
    assert '"unresolved_from": 2' in saved[1]


def test_fill_submitted_appends_when_no_suggestion_row(monkeypatch):
    """扫描件没跑过(或那行已被裁掉)⇒ 整行追加,流水不能丢。"""
    conn, wrote = _sheet_env(monkeypatch, [], {"next_row": 2, "unresolved_from": 2})
    filled, added = maint_sheet.fill_submitted([
        _sheet_row("T1", "B0Z", "删除", "", "", "F1", "2026-08-17", "处理中",
                   suggestion="删除")])
    assert (filled, added) == (0, 1)
    assert [r for r, _ in wrote] == ["A2:K2"]      # 整行


def test_fill_submitted_settles_put_path_in_place(monkeypatch):
    """PUT 同步路径(feedid='sync')当场就知道结果 —— 反哺器永远不会来管它。"""
    existing = [_sheet_row("T1", "B0A", "", "", "", "", "2026-08-17", "",
                           suggestion="库存")]
    conn, wrote = _sheet_env(monkeypatch, existing,
                             {"next_row": 3, "unresolved_from": 2})
    maint_sheet.fill_submitted([
        _sheet_row("T1", "B0A", "库存", "5", "0", "sync", "2026-08-17", "成功",
                   suggestion="库存")])
    rngs = [r for r, _ in wrote]
    assert rngs == ["E2:H2", "J2:K2"]              # 两段,跳过中间的日期列
    assert wrote[1][1] == [["成功", ""]]


def test_append_suggestions_skips_rows_already_waiting(monkeypatch):
    """扫描件可随时跑:同 (店铺,SKU,建议) 已有未执行行就不重复写。
    ...
    conn, wrote = _sheet_env(monkeypatch, existing,
                             {"next_row": 3, "unresolved_from": 3})
    written, skipped = maint_sheet.append_suggestions([
        ("T1", "B0A", "价格", "亚马逊涨价", "2026-08-17"),     # 已在表里
        ("T1", "B0B", "删除", "标题不符", "2026-08-17"),       # 新的
    ])
    assert (written, skipped) == (1, 1)
    assert [r for r, _ in wrote] == ["A3:K3"]
    # ⚠ 按名字断言:扫描件写的五列在表里**不相邻**(日期是第 9 列),
    # 写死 [:5] 会把「动作」当成日期
    got = wrote[0][1][0]
    assert [got[maint_sheet._idx(c)] for c in
            ("store", "sku", "suggestion", "reason", "op_date")] == \
        ["T1", "B0B", "删除", "标题不符", "2026-08-17"]


def test_append_suggestions_does_not_dedupe_against_executed_rows(monkeypatch):
    """已执行过的行(动作已填)不参与查重 —— 明天再次被建议该另起一行,
    而不是把昨天那条已执行的记录当成"还在等着"。"""
    existing = [_sheet_row("T1", "B0A", "价格", "10", "12.5", "F1",
                           "2026-08-16", "成功", suggestion="价格")]
    conn, wrote = _sheet_env(monkeypatch, existing,
                             {"next_row": 3, "unresolved_from": 3})
    written, skipped = maint_sheet.append_suggestions([
        ("T1", "B0A", "价格", "亚马逊又涨价", "2026-08-17")])
    assert (written, skipped) == (1, 0)


def test_scan_row_carries_a_date_so_prune_can_age_it_out(monkeypatch):
    """扫描件必须写日期:建议了却一直没执行的行(撞上单店删除上限的那些)
    靠 prune 按日期老化,不写日期它们永远裁不掉,表只涨不减。"""
    conn, wrote = _sheet_env(monkeypatch, [], {"next_row": 2, "unresolved_from": 2})
    maint_sheet.append_suggestions([("T1", "B0A", "删除", "标题不符", "2026-08-17")])
