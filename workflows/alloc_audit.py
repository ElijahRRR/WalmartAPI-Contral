...
**代码分三段**(2026-08-27 拆):`_collect` 取全部事实与派生量装进 `Ctx` →
`_sections` 拼控制台六节 → `_export` 落 csv 并补报告尾。三段读**同一份**
`Ctx`:冲突判定(`conflicts`)与占用台账(`held`)只算一次,报告与 csv 共用
—— 两边各算一遍,口径纪律 2 就没人守得住了。
"""
...
def _n(v) -> str:
    """输入:数 → 输出:千分位串(报告里所有计数共用这一个写法)。"""
    return f"{int(v):,}"


@dataclass
class Ctx:
    """一轮审计的全部事实与派生量 —— 报告段与 csv 段读的是同一份。

    ⚠ 三个 `*_err` 与它们的值是**成对**的:值为 None 表示「本轮没查到」,
    与"查了确实是空"不是一回事。报告要分得开,合成一个值就会把"没查"
    印成"没有"(这份报告最容易骗人的地方)。
    """
    # ── 参数 ──
    with_channel: bool
    ...
    win: dict
    export: bool
    # ── 库里的事实(services/alloc_survey.load_rows 的产物)──
    rows: list
    st: Counter
    pool: dict
    sig: dict
...
def _collect(params: dict) -> Ctx:
    """输入:params(channel/sales_days/as_of/export)→ 输出:本轮的 Ctx。"""
    ...
    with db.pg_conn() as conn:
        # 四条分配链的唯一装载口(私有 _SQL_* 留在服务内);本件要全套探针
        loaded = sv.load_rows(conn, win=win, need=sv.NEED_ALL,
                              with_channel=with_channel)
    rows, st = loaded.rows, loaded.st
    ...
    # 各节要用的数在这里一次算完 —— 报告段与 csv 段读同一份,不许各算一遍。
    ...
    non_active = sorted(s for s in prof_all
                        if (loaded.status.get(s) or "ACTIVE") != "ACTIVE")
    filtered = (sorted((set(registered) & set(prof_all)) - live_api)
                if registered is not None and live_api is not None else [])
    order_miss = ([(s, c, h) for s, c, h in loaded.order_stores
                   if s not in registered] if registered is not None else [])

    return Ctx(
        with_channel=with_channel, sales_days=sales_days, win=win, export=export,
        rows=rows, st=st, pool=loaded.pool, sig=loaded.signal,
        dic=loaded.pt_dict, dic_err=loaded.pt_dict_err, cats=loaded.categories,
        status=loaded.status, sales=sales, order_stores=loaded.order_stores,
        registered=registered, reg_err=reg_err, live_api=live_api,
        live_err=live_err, cfg=cfg, cfg_err=cfg_err, held=held,
        held_err=held_err,
        prof_all=prof_all, prof=prof, live_rows=live_rows, frozen=frozen,
        out_of_scope=out_of_scope, oos_rows=oos_rows, slot_rows=slot_rows,
        claim_rows=claim_rows, metrics=metrics, conflicts=conflicts,
        hard=hard, drop_all=drop_all, over=over, can_channel=can_channel,
        mism=mism, offenders=offenders, cat_bad=cat_bad,
        cat_unknown=cat_unknown, scope=scope, miss_cfg=miss_cfg,
        empty_stores=empty_stores, unfilled_cat=unfilled_cat,
        opted_out=opted_out, non_active=non_active, filtered=filtered,
        order_miss=order_miss)


# ══ 控制台六节:只放"一眼能扫完的结论 + 要做的事",明细全进 csv ══

def _section_todo(c: Ctx) -> list[str]:
    """▍要你做的事(放最前面:报告是为了让人动手,不是为了让人读)。"""
    todo = [("① 填类目三列",
             f"{len(c.unfilled_cat)} 家店待填" if not c.cfg_err else "本轮没查",
             "→ alloc_类目建议.csv" if not c.cfg_err else
             f"**跳过**:限额表读不到({c.cfg_err}),已填值无从对拍"),
            ("② 下架渠道不符",
             f"{len(c.offenders)} 件" if c.can_channel else "本轮没查",
             "→ alloc_渠道不符下架清单.csv" if c.can_channel else
             ("**跳过**:-p channel=0 没取渠道,去掉该参数重跑才有结论"
              if c.with_channel is False else "**跳过**:限额表读不到,没有配送限制可对拍")),
            ("③ 下架类目不符",
             f"{len(c.cat_bad)} 件" if c.cfg else "本轮没查",
             "→ alloc_类目不符下架清单.csv" if c.cfg else
             "**跳过**:限额表读不到,没有准入类目可对拍"),
            ("④ 确认冲突处置",
             f"要下架 {c.drop_all} 件"
             f"(同 ASIN {len(c.conflicts['同 ASIN'])} 组 · "
             f"同品牌 {len(c.conflicts['同品牌'])} 组)",
             "→ alloc_同ASIN冲突处置.csv 等 2 份"
             + (f",其中 {c.hard} 组机器判不出、要人眼看" if c.hard
                else ",全部可自动判"))]
    if c.miss_cfg:
        have = [s for s in c.miss_cfg if s in c.prof]
        todo.append(("⑤ 补店铺配置", f"有货店缺列 {len(have)} 家",
                     "→ alloc_店铺总览.csv「缺配置列」"
                     + (f";另有 {len(c.empty_stores)} 家空店没登记,"
                        f"要它们参与分配才用填" if c.empty_stores else "")))
    ...
    if c.cfg and not c.with_channel:
...
def _section_health(c: Ctx) -> list[str]:
    """▍数据体检:候选池逐层收窄 / 打分信号 / PT 两本字典对拍 / 品牌键。"""
    body = [("候选池", f"产品 {_n(c.pool['total'])} → 审核通过 "
                       f"{_n(c.pool['approved'])} → **可分配 "
                       f"{_n(c.pool['with_cat'])}**;其中 PT 实证 "
                       f"{_n(c.pool['pt_evid'])} / 推断 "
                       f"{_n(c.pool['with_pt'] - c.pool['pt_evid'])}"),
            ("打分信号", f"评分 {'✓' if c.sig['n_rating'] else '✗'} · "
                         f"评论 {'✓' if c.sig['n_review'] else '✗'} · "
                         f"渠道 {'✓' if c.sig['n_fba'] else '✗'}(近 5 万快照)"
                         + ("——**评分/评论为空,v1 权重要删掉这两项**"
                            if not c.sig["n_rating"] else ""))]
    if c.dic_err:
        body.append(("PT 字典", f"对拍跳过({c.dic_err})"))
    else:
        same = (c.dic["only_risk"] == c.dic["only_meta"] == c.dic["cat_diff"] == 0)
        body.append(("PT 字典",
                     f"两本各 {_n(c.dic['n_risk'])}/{_n(c.dic['n_meta'])} 条,"
                     + ("完全一致" if same else
                        f"**有差异:仅前者 {c.dic['only_risk']} / 仅后者 "
                        f"{c.dic['only_meta']} / 大类不一致 {c.dic['cat_diff']}**")
                     + f";大类 {len(c.cats)} 个"))
    n_key = sum(1 for r in c.rows if r["brand_key"])
    body.append(("品牌键", f"可占用 {_n(n_key)} 行 · 真·无品牌 {_n(c.st['no_brand'])}"
                           f" · 产品库缺行 {_n(c.st['asin_not_in_products'])}"))
    return ["", "▍数据体检"] + textfmt.grid(body)


def _section_online(c: Ctx) -> list[str]:
    """▍在线商品:规划内多少、排除了谁、数据缺口多少。"""
    n_pub = sum(1 for r in c.rows if r["published"])
    body = [("规划内", f"{len(c.prof)} 家店 {_n(len(c.live_rows))} 行")]
    if c.out_of_scope:
        body.append(("已排除", f"规划外 {len(c.out_of_scope)} 家 {_n(c.oos_rows)} 行"
                               f"({'、'.join(sorted(c.out_of_scope)[:6])})"
                               f" —— 不占用、不算冲突,**销量仍计入**"))
    if c.frozen:
        body.append(("已排除", f"不在营店冻结 {len(c.frozen)} 家"
                               f"({'、'.join(sorted(c.frozen)[:6])})"
                               f" —— 整店释放见 store_release"))
    if c.registered is None:
        body.append(("⚠ 未排除", f"凭证表读不到({c.reg_err}),幻影店一个没排,"
                                 f"下面的冲突数只可参考"))
    unpub = sum(1 for r in c.live_rows if not r["published"])
    body.append(("不占货位", f"未发布 {_n(unpub)} 行 —— 在店里但没上架,"
                             f"不算占着货位,冲突与回填都不算它"))
    body.append(("数据缺口", f"归不到大类 {_n(c.st['no_category'])} 行 · "
                             f"SKU 提不出 ASIN {_n(c.st['no_asin'])} 行"))
    L = ["", f"▍在线商品 {_n(c.st['online'])} 行(已发布 {_n(n_pub)} / "
             f"未发布 {_n(c.st['online'] - n_pub)};RETIRED 退市行已在 SQL 排除)"]
    return L + textfmt.grid(body)


def _section_conflicts(c: Ctx) -> list[str]:
    """▍冲突:同 ASIN / 同品牌各多少组,按判定依据分布。"""
    body = []
    ...
    if c.held_err:
        L.append(f"  ⚠ **占用台账读不到({c.held_err}):本轮没看占用**,上面全是"
                 f"按销量阶梯判的。已经有主的品牌可能被判给别人 —— "
                 f"**先修好再照着做**")
    elif c.held is not None:
        n_held = sum(len(v) for v in c.held.values())
        stuck = [x for tag in ("同 ASIN", "同品牌")
                 for x in c.conflicts[tag] if x[4] == sv.CLAIM_STUCK]
        by_claim = sum(1 for tag in ("同 ASIN", "同品牌")
                       for x in c.conflicts[tag] if x[4] == sv.BY_CLAIM)
        if not n_held:
    ...
    if not c.sales:
        ...
        L.append(f"  ⚠ **订单历史还没导入**(近 {c.sales_days} 天 0 个 (店,SKU) "
                 f"组合),上面全是按在线件数打平的,不是按销量判的;"
                 f"先跑 order_history_import -p apply=1 再重跑本报告")
    elif not c.hard and (c.conflicts["同 ASIN"] or c.conflicts["同品牌"]):
        L.append("  ✓ 全部有销量/类目依据,不需要人工逐条看")
    return L


def _section_stores(c: Ctx) -> list[str]:
    """▍店铺:超 2 品类、非 ACTIVE、凭证被过滤、渠道/类目有不符,各多少家。"""
    body = []
    if c.over:
        body.append(("超 2 品类", f"{len(c.over)} 家", "最碎:" + "、".join(
            f"{s}({len(sv.real_supers(p))} 品类 / {len(sv.real_cats(p))} 个 26 类)"
            for s, p in c.over[:3])))
    if c.non_active:
        body.append(("非 ACTIVE", f"{len(c.non_active)} 家",
                     "SUSPENDED,**占用按设计保持不动**;"
                     "要它暂时不接新货,把「单店最大在线数」填 0"))
    if c.cfg:
        body.append(("不参与分配", f"{len(c.opted_out)} 家",
                     ("「单店最大在线数」填 0 —— 所有者的开关,核对是不是这几家:"
                      + "、".join(c.opted_out[:6])) if c.opted_out else
                     "没有店填 0;**要哪家不接货就把这一列填 0**"))
    if c.filtered:
        body.append(("凭证被过滤", f"{len(c.filtered)} 家",
                     "缺代理/ClientId,**不是死店**,别整店释放"))
    if c.mism:
        body.append(("渠道有不符", f"{len(c.mism)} 家", f"共 {len(c.offenders)} 件待下架"))
    if c.cat_bad:
        by = Counter(r["store"] for r in c.cat_bad)
        body.append(("类目有不符", f"{len(by)} 家", f"共 {len(c.cat_bad)} 件待下架;"
                     + "、".join(f"{s}×{x}" for s, x in by.most_common(3))))
    L = ["", f"▍店铺(规划内有货 {len(c.prof)} 家)"]
    return L + (textfmt.grid(body) if body else ["  ✓ 没发现异常"])


def _section_notes(c: Ctx) -> list[str]:
    """▍要留意:会让上面的数字读歪的那些事。"""
    notes = []
    if c.cat_unknown:
        notes.append(f"归不到大类 {_n(c.cat_unknown)} 行(在准入受限的店里)—— "
                     f"**没进类目不符清单**:那是「不知道它属于哪类」,"
                     f"不是「属于不该有的类」,别拿它当违规下架")
    if c.st["channel_weird"]:
        notes.append(f"渠道值认不出 {_n(c.st['channel_weird'])} 行 —— "
                     f"采集侧 is_fba 没给值,不是货不对,**别下架**")
    if c.order_miss:
        notes.append(f"订单店名对不上 {len(c.order_miss)} 家 "
                     f"{sum(x for _, x, _ in c.order_miss)} 行("
                     + "、".join(f"{s or '(空店名)'} {x}"
                                 for s, x, _ in c.order_miss[:4])
                     + ")—— 这些销量进不了店×类目维度")
    if c.live_err:
        # 读不到就没有「配置缺失 vs 真不在册」这一栏,而不是"一家都没有"
        notes.append(f"店铺配置读不到({c.live_err}),本轮没法区分"
                     f"「缺代理/ClientId 的在营店」和「已停用的死店」")
    if not notes:
        return []
    return ["", "▍要留意"] + [f"  · {x}" for x in notes]


def _sections(ctx: Ctx) -> list[str]:
    """输入:Ctx → 输出:控制台六节的全部文本行(顺序即所有者读的顺序)。"""
    L: list[str] = ["", "═══ 分配存量审计 ═══"]
    for section in (_section_todo, _section_health, _section_online,
                    _section_conflicts, _section_stores, _section_notes):
        L += section(ctx)
    return L


# ══ 明细 csv:控制台只留计数,照着动手的东西全在这里 ══

def _csv_category_advice(c: Ctx) -> list[str]:
    """▍alloc_类目建议.csv —— 每店 top2 品类 + 与已填值对拍。"""
    sug = sv.suggest_categories(c.prof, c.cfg)
...
def _csv_store_overview(c: Ctx) -> list[str]:
    """▍alloc_店铺总览.csv —— 逐店明细一次给全,控制台只留计数。"""
    ...
    return [report_csv.write(
        ...
        [(s,
          ...
          # 填 0 让它暂时不接新货(参与分配=否)
          "否(规划外)" if s in c.out_of_scope else
          ("否(不在营)" if s in c.frozen else "是"),
          _ACCEPT[store_targets.accepts_allocation(c.cfg.get(s))] if c.cfg else "",
          "" if (c.cfg.get(s) or {}).get("max_online") is None
             else int(c.cfg[s]["max_online"]),
          p["n"], p["published"],
          len(sv.real_supers(p)), "|".join(sv.real_supers(p)),
          len(sv.real_cats(p)),
          "; ".join(f"{cat}×{x}" for cat, x in p["categories"].most_common(3)
                    if cat != sv.UNCLASSIFIED),
          (c.cfg.get(s) or {}).get("channel") or "",
          next((m[2] for m in c.mism if m[0] == s), 0),
          "|".join((c.cfg.get(s) or {}).get("categories") or []),
          # 准入品类:闸真正看见的东西。与左边一列并排,人才能一眼看出
          # 「填了 Home + Furniture」其实只开了一个 Home
          "|".join(sorted(store_targets.super_categories_of(c.cfg.get(s))))
          if c.cfg else "",
          c.status.get(s) or "(无KPI)",
          "/".join(c.miss_cfg.get(s, [])),
          round((c.metrics[0].get(s) or {}).get("gmv", 0.0), 2))
         for s, p in sorted(c.prof_all.items(),
                            key=lambda kv: -kv[1]["n"])])]


def _csv_conflicts(c: Ctx) -> list[str]:
    """▍两份冲突处置清单 —— 与报告读同一份 conflicts/held,不重算。"""
    files = []
    for tag, fname in (("同 ASIN", "alloc_同ASIN冲突处置.csv"),
        ...
        files.append(report_csv.write(
            ...
            ["冲突键", "组内店铺数", "组内行数", "已有占用", "保留店", "判定依据",
             ...
             f"近{c.sales_days}天单量", "该商品销售额", "该店该大类销售额",
             "该店整体销售额", "处置"],
            [(key, len({d[0] for d in detail}), len(detail),
              ("(本轮没读到)" if c.held is None
               else (c.held[tag].get(key) or "(无)")),
              keep, level, s2, sku, asin, cat, resources.super_label(cat),
              o, g, cg, sg, verdict)
             for key, keep, _, detail, level in c.conflicts[tag]
             for s2, sku, asin, cat, o, g, cg, sg, verdict in detail]))
    return files


def _export(c: Ctx) -> list[str]:
    """输入:Ctx → 输出:落 csv 之后追加的报告行(`-p export=0` 时只说没落)。"""
    if not c.export:
        return ["", "(-p export=0:未落明细 csv)"]

    files = []
    if c.cfg is not None and not c.cfg_err:
        files += _csv_category_advice(c)
    if c.can_channel:
        files += _csv_offender_lists(c)
    files += _csv_store_overview(c)
    files += _csv_conflicts(c)

    # 销量窗口必须打出来:阶梯前三级全看销量,窗口一滑判定就可能翻。
    # 回填要照这份清单落,就得吃同一个窗口 —— 命令原样给出,不让人自己拼
    L = ["", f"▍销量窗口 {c.win['day']} 往前 {c.sales_days} 天(右端不含当天)",
         f"  回填照这份清单落:python cli.py alloc_backfill "
         f"-p as_of={c.win['day']} -p sales_days={c.sales_days}"]
    L += ["", f"▍明细 {len(files)} 份 csv → {paths.reports_dir()}"]
    for f in files:
        L.append(f"  · {f.rsplit('/', 1)[-1]}")
    if c.cfg_err:
        L.append(f"  ⚠ 限额表读不到({c.cfg_err}):类目建议与渠道清单本轮没出")
    return L
...
def run(params: dict) -> str:
    ...
    ctx = _collect(params)
    return "\n".join(_sections(ctx) + _export(ctx))
