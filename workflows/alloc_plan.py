...
def run(params: dict) -> str:
    ...
    # 产品会**跟着一起上架**。所有者 2026-08-16 追问「产品分 38.6 和 91.4 怎么混
    # 到一起去的」正是这件事,并据此定了搭车地板(alloc_groups.RIDE_FLOOR)。
    # 地板挡不住的还剩一种:**整组都低于地板**的组(没车可搭,组队时不剪),
    # 它们被发出去时组里仍有低分件。剩多少要报出来 —— 归零了才敢说地板生效。
    if deck:
        floor = deck[-1]["score"]
        ...
        if ride:
            L.append(f"  其中 {ride:,} 个货位是**跟着同品牌高分品搭车上架**的"
                     f"(产品分低于本轮切口 {floor:.1f})—— 搭车地板 "
                     f"{alloc_groups.RIDE_FLOOR:.0f} 分只剪"
                     f"「组里有 ≥{alloc_groups.RIDE_FLOOR:.0f} 分的品」那种组,"
                     f"整组都低于地板的没车可搭、一件不剪,剩的就是这批。"
                     f"方案表的「组分 / 组件数 / 产品分」三列并排,可以逐组核")
    if dir_trim:
