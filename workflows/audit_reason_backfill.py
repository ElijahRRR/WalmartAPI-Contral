...
⚠ **2026-09-02 第三步 B1 之后它写的是老格式**(规格 §3.4「存量结论不迁移」):
B1 起 `audit_reason` 只装**类别**枚举、具体内容改落新列 `audit_detail`,而本
工作流仍把「历史结论(阶段 X):<人话>」写进 `audit_reason` —— 它动的**只有
B1 之前的存量占位行**,与老行的渲染口径(飞书 H 列按"没有 audit_detail 就按
命中规则渲染"兜底)一致,所以不算错位。**不扩它**:存量行的正解是被重审时
自然写上新格式,不是再回填一遍(所有者定稿:按需重审为主)。
