...
⚠ **`LEGACY`(项目初期一次性导入的历史行)不特殊对待**(所有者 2026-09-06:
「删」)。判不出的(`taxonomy_code` NULL,10,020 条)照上面那条**留**;找到了
当年全文、判出非永久的(实测 339 条:PT_WRONG 302 / EXPIRED 28 / STAGE 6 /
PRICE 3)**删** —— 同一段沃尔玛原文,不能因为进表的路径不同而去留不同。
昨天那句「历史导入需要保留」说的是 `rebuild_asin` **删了灌不回来**,与这里
按全文判定、且删前落备份,是两件事。
...
1. **只路由回填过的行**:`taxonomy_version = <当前 ERROR_TAXONOMY_VERSION>`。
   ...
   提示先跑 `error_reclass`。⚠ 2026-09-07 起日常进口(`record_asins`)也
   盖章,所以这一栏正常应为 0;不为 0 = 码表刚递增、或有行绕过了进口;
