...
WHERE d.walmart_product_type NOT IN ('无对应Walmart PT', '-', '')
...
def run(params: dict) -> str:
    ...
    with db.pg_conn() as conn:
        ...
        rows = [r for r in all_rows if r[1] not in alive]
        ...
        lines = [f"{'' if execute else '🧪 [DRY-RUN] '}"
                 f"指向「不在{src}」的 PT 的映射:{len(rows)} 行 / "
                 f"{len(pts)} 个 PT / {paths} 条路径",
                 f"  判据 {len(alive)} 个真实 PT;处置方式:{mode}"]
        if orphan:
            lines.append(
