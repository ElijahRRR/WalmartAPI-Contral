...
闸门链(顺序即执行序,每道命中写「未上架理由」列或摘要计数):
  ...
  ⑥ 数据过滤:亚马逊库存 <5 淘汰(理由「亚马逊库存不足5」,数字随门槛
    常量);过了门槛,上架数量 = min(亚马逊库存, 限额表本店「最大库存」N),
    没设 N 就是亚马逊原数(所有者定稿 2026-09-25,换算唯一实现
    ...
    for r in cands:
        ...
        if stock is None:
            if p.get("stock_state") == "in_stock":
                ...
                continue
        # 门槛决定**上不上**,本店「最大库存」N 决定**上多少**(所有者定稿
        # 2026-09-25):低于门槛不上架,理由写门槛的值(门槛以后可能调,
        # 数字跟着常量走);过了门槛,上架数量 = min(亚马逊库存, N)
        qty, qty_why = store_limits.stock_for(stock,
                                              ctx.stock_caps.get(store_name))
        ...
        if gap:
            ...
            continue
        # 上架数量 = 上面 stock_for 的换算结果(亚马逊库存超过 N 时是 N,
        # 其余是亚马逊原数)。封顶计数放在这里而不是换算处:被后面几道闸拦掉
        # 的行不算"按上限上架了"(「按 10 铺货」那个计数就是算早了的反例)
...
def run(params: dict) -> str:
    ...
    if n["stock_assumed"]:
        ...
        gate_line += (f";库存数未采到按 {amz_source.IN_STOCK_QTY} 铺货"
                      f" {n['stock_assumed']} 行")
    if n["stock_capped"]:
        # 设了「最大库存」的店:亚马逊库存超过 N 的行按 N 上架,不是亚马逊原数
