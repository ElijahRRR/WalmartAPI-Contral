...
意图来源(services/maintenance_intents,可插拔 provider,采集侧改造中):
  清零   ✅ 做实:限额表「库存特殊要求」=0 的 stockzero 店整店清零(不依赖采集;
         所有者定稿:不设清零二次确认)
  改价/改库存/改标题 ⏳ 预留接口:采集(catalog.latest_snapshot)接入后填实,
         管道零改动;涨跌幅闸位留在 price provider(暂不需要)
...
def collect_intents(conn, stockzero: list[str]) -> list[dict]:
    ...
    intents = []
    intents += mi.title_intents(conn)
    intents += mi.price_intents(conn)
    intents += mi.inventory_intents(conn)
...
def run(params: dict) -> str:
    ...
    if not intents:
        return ("无维护意图(改价/改库存/改标题待采集服务接入;"
