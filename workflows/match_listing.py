...
生成 upc/gtin 候选依次试)→ 三路:
  MP_ITEM_MATCH(已在售)→ 可跟卖 → **三道闸**(2026-08-12 接入,补齐
...
三道闸(跟卖无 amz 侧身份,全靠 SPEC 交叉字段;交叉不出的字段跳过
该道闸,不是放行整行;命中写 F=终态,运营核对后清 F 重新排队):
  ...
  ② ASIN 黑名单:SPEC 交叉出的 ASIN 命中 catalog.asin_blacklist(永久
    产品级禁止——被删过的产品换跟卖通道回来,拦的就是这个)
  ③ 防呆 product_risk:交叉 ASIN 有删除史;或同 GTIN 的旧跟卖 offer
    有删除史(经 listing_sources 把 GTIN→历史 sku→病历接回来——跟卖
    sku 是自编号,换个新编号重跟按 ASIN 查必然漏)
...
_TERMINAL = ("需完整建品", "目录无", "预检失败", "店铺不识别", "码无效",
             "风控拦截", "ASIN黑名单", "防呆")     # 后三类为前缀


def _gate_reason(spec: dict, gate: dict, banned: dict, risky: dict,
                 risky_keys: set) -> str | None:
    """输入:SPEC 预检结果 + 三道闸数据 → 输出:拦截原因(None=放行)。
    ...
    if asin and asin in risky:
        return product_events.risk_reason(*risky[asin])
    pid = spec.get("product_id")
    if pid and pid in risky_keys:
        return "防呆:该GTIN的旧跟卖offer有删除史"
    return None
...
def run(params: dict) -> str:
    ...
    # + 三道闸数据每轮加载一次,逐行零查询(与 list_new 同款)
    ...
    with db.pg_conn() as conn:
        ...
        risky_keys = listing_sources.risky_source_keys(
            conn, listing_sources.SOURCE_MATCH)
    ...
    for r in todo:
        ...
        if pre["status"] != "可跟卖":
            ...
            continue
        spec = pre["spec"]
        why = _gate_reason(spec, gate, banned, risky, risky_keys)
        if why:
