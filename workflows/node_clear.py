...
**判断条件**(所有者确认 2026-09-25):**维护仓里有这个 SKU 的库存记录(数量 0
也算),且旧仓有货** → 这个旧仓写 0。

  | 维护仓   | 旧仓 | 处理 |
  | 3        | 999  | 清 |
  | 0        | 999  | 清 —— 维护链按规则写的 0(亚马逊缺货/不足门槛),旧仓的
  |          |      |   货还在卖;按"多个仓都有货"判断会漏掉这一类 |
  | 没有记录 | 999  | 不清 —— 维护仓还没接管,清了它在所有仓都是 0,直接断售
  |          |      |   (谭总12 搬仓当天实见 112 个未接管) |
...
⚠ 库里是**上一次 catalog_sync** 的数据(每天 06:40、13:00)。当天刚被受管仓
接管的 SKU 要等下一次同步后才会被清(方向安全);库里显示旧仓有货、实际已经
清空的,再写一次 0 也没有影响。同一批载荷还在处理中时重跑,feed 台账的防重会
拦下(`dedup`),不会重复提交。
...
# 该店受管仓以外、有货的「SKU × 仓」,顺带带出"受管仓有没有这个 SKU 的记录"。
...
SELECT n.sku, n.ship_node, n.avail_qty,
       EXISTS (SELECT 1 FROM catalog.item_node_inventory m
               WHERE m.store = n.store AND m.sku = n.sku
                 AND m.ship_node = %(managed)s::text) AS taken
...
def plan(rows: list[tuple]) -> tuple[dict[str, list[str]], dict, dict, int]:
    """输入:[(sku, 旧仓, 数量, 受管仓有无记录)] → 输出:(待清 {旧仓: [sku…]},
    {旧仓: (待清 SKU 数, 件数)}, 未接管 {sku: 旧仓合计件数}, 节点身份未知的份数)。

    纯函数:判断条件全在这里(见模块头注的表),run() 只管读、发、报。
    """
    targets: dict[str, list[str]] = {}
    per_node: dict[str, tuple[int, int]] = {}
    untaken: dict[str, int] = {}
    n_unknown = 0
    for sku, node, qty, taken in rows:
        if not node:
            n_unknown += 1          # 同步时接口没给 shipNode:没有节点可写
            continue
        if not taken:
            untaken[sku] = untaken.get(sku, 0) + int(qty)
            continue
...
def _clear_store(store: dict, managed: str, preview: bool) -> dict:
    ...
    with db.pg_conn() as conn, conn.cursor() as cur:
        ...
        rows = cur.fetchall()
    targets, per_node, untaken, n_unknown = plan(rows)
    ...
    if untaken:
        lines.append(
            f"    受管仓还没有记录、本轮不清 {len(untaken)} 个(旧仓合计 "
            f"{sum(untaken.values())} 件),样本:{sorted(untaken)[:5]}"
            "(维护链往受管仓写过它之后,下次同步完再跑一遍就会清)")
    if n_unknown:
