...
真跑的**连带后果两条**(摘要逐条点名;都不会自己发生,也都不会报错):
  ...
  ① `AUDIT_RULES_VERSION` **已由本批递增(见 registry/resources.py),首跑无需
     ...
     18:00–次日 08:00 的谷时段(或周末全天)跑,单价减半
     (registry/resources.LLM_PRICING);
  ② 新增行的人工中文列全是 NULL —— 但**判据是完整的、立刻生效**:B1(2026-09-02)
     之后 S4 读的是 `full_policy`(官方英文全文),中文人工列**判定链一处都不读**。
     中文列只服务人读面(飞书政策表投影),运营没填不影响判定。
     ⚠ 反过来才要紧:`full_policy` 为空的行会被 S4 **整条跳过**(判据里根本没有它),
     摘要与 `product_audit` 的「L3 缺全文」计数会点名是哪几条。
     (原文写的是「S4 渲染成空壳标题、等运营填中文才有用」—— 那是 B1 之前 S4 读
      六个中文列的口径,2026-09-03 随换喂一并作废,别照着它去等运营。)
...
def run(params: dict) -> str:
    ...
    if execute:
        ...
        lines.append("     ⇒ 成本:L3 的 system prompt 随之逐字节变化,"
                     ...
                     "谷时段,单价减半;")
        lines.append(f"  ② 新增 {n_new} 行的人工中文列全是 NULL,但**判据完整、"
                     "立刻生效**:S4 读的是 full_policy(官方英文全文),中文人工列"
                     "判定链一处都不读;中文列只服务人读面(飞书政策表),"
                     "运营没填不影响判定。"
                     "⚠ 真正要紧的是反面:full_policy 为空的行会被 S4 整条跳过 —— "
                     "本轮 44 行都有全文,跑 product_audit 时看摘要的"
                     "「L3 缺全文」计数应为 0。")
    else:
